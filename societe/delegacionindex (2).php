...
print load_fiche_titre($langs->trans(utf8_encode("Listado Delegaci�n")), '', 'companies');
...
print "
	...
	<tbody>
		...
		<tr class='liste_titre_filter'>
			...
			<td class='wrapcolumntitle liste_titre middle'>
				<input class='flat maxwidth80imp' type='text' name='search_tel1'>
			</td>
			<td class='wrapcolumntitle liste_titre middle'>
				<input class='flat maxwidth75imp' type='text' name='search_direccion'>
			</td>
			<td class='wrapcolumntitle liste_titre middle'>
				<input class='flat maxwidth75imp' type='text' name='search_provincia'>
			</td>
			<td class='wrapcolumntitle liste_titre middle'>
				<input class='flat maxwidth75imp' type='text' name='search_localidad'>
			</td>
		...
		<tr class='liste_titre'>
			...
			<th class='wrapcolumntitle liste_titre' title='Telefono'>
				<a class='reposition' href=''>" .utf8_encode("Tel�fono") . "</a>
			</th>
			<th class='wrapcolumntitle  liste_titre' title='Direccion'>
				<a class='reposition' href=''>" .utf8_encode("Direcci�n") . "</a>
			</th>
			<th class='wrapcolumntitle  liste_titre' title='Provincia'>
				<a class='reposition' href=''>Provincia</a>
			</th>
			<th class='wrapcolumntitle liste_titre' title='localidad'>
				<a class='reposition' href=''>Localidad</a>
			</th>
		...
		if (isset($_POST['button_search'])) {
			...
			if (isset($_POST['search_tel1']) && ($_POST['search_tel1']) != "") {
				$tel1 = "'" . $_POST['search_tel1'] . "'";
				$sql .=' and telef1='.$tel1;
			}
			...
			if (isset($_POST['search_provincia']) && ($_POST['search_provincia']) != "") {
				$provincia= "'" . $_POST['search_provincia'] . "'";
		...
		while($aux<$num){
			...
			print '<a class="editfielda" href="delegacion.php?action=editar&id='; print $obj->id.'">
	...
	<div tabindex="-1" role="dialog" class="ui-dialog ui-corner-all ui-widget ui-widget-content ui-front ui-dialog-buttons ui-draggable" aria-describedby="dialog-confirm" aria-labelledby="ui-id-1" style="height: auto; width: 500px; top: 268.503px; left: 457.62px; z-index: 101;">
		...
		<div id="dialog-confirm" style="width: auto; min-height: 0px; max-height: none; height: 97.928px;" class="ui-dialog-content ui-widget-content">
			...
			<div class="confirmmessage">
				...
				' . utf8_encode("�Est� seguro ") . 'de querer eliminar la ' . utf8_encode("delegaci�n") .': '; echo $nombre.'?
			</div>
		</div>
...
if (isset($_POST['confirmmassaction'])) {
	...
	$respuesta = $db->query($sql);
	if($respuesta ){
		setEventMessages( utf8_encode("Delegaci�n") .' eliminada', null, 'mesgs');
		header('Location: /societe/delegacionindex.php');
        die();
	}else{
		setEventMessages( 'Error al eliminar la delegaci�n', null, 'errors');
	}
}
