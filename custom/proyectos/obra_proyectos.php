...
$help_url = '';
llxHeader('', $langs->trans('Proyectos'), $help_url);
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'proyectos', '', -1, $object->picto);
	...
	print_barre_liste($langs->trans("Proyectos"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'members', 0, $newcardbutton, '', $limit, 0, 0, 1);
	...
	if (!empty($arrayfields['codigo']['checked'])) {
		...
		print "<a class='reposition' href=''>Nº proyecto</a>";
		print "</th>";
	}
	...
	if (!empty($arrayfields['fin_garantia']['checked'])) {
		...
		print "<a class='reposition' href=''>Realizado por</a>";
		print "</th>";
	}
	...
	if (isset($_POST['button_search'])) {
        ...
        if (isset($_POST['search_percentaje_discount']) && ($_POST['search_percentaje_discount']) != "") {
            $importe = "" . $_POST['search_percentaje_discount'] . "";
            $sql .= ' and amount = ' . $importe . '';
        }

        if (isset($_POST['search_total']) && ($_POST['search_total']) != "") {
			$importe = "" . $_POST['search_total'] . "";
			$sql .= ' and amount = ' . $importe . '';
		}
	}
	...
	while ($i < $num) {
        ...
        if (!empty($arrayfields['percentaje_discount']['checked']))	print "<td class='center' tdoverflowmax200'>". $datos->ubicacion ."</td> ";
		
        if (!empty($arrayfields['total']['checked']))	print "<td class='center' tdoverflowmax200'>". $datos->ubicacion ."</td> ";
