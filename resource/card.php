...
if ($action == 'create' || $object->fetch($id, $ref) > 0) {
	...
	if ($action == 'create' || $action == 'edit') {
		...
		// Type
		print '<tr style="display:none"><td>'.$langs->trans("ResourceType").'</td>';
		print '<td>';
		$ret = $formresource->select_types_resource($object->fk_code_type_resource, 'fk_code_type_resource', '', 2);
		print '</td></tr>';

		// Description
		print '<tr style="display:none"><td class="tdtop">'.$langs->trans("Description").'</td>';
		print '<td>';
		...
		// Origin country
		print '<tr style="display:none"><td>'.$langs->trans("CountryOrigin").'</td><td>';
