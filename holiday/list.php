...
if(empty($search_employee)&&$user_is_admin==0){
...
//MIRAR ESTO
if ($id>0) {
	$sql .= " AND cp.fk_user IN (".$db->sanitize($id).") or cp.fk_validator= $search_employee";
}
...
if ($resql) {
	...
	if (!empty($arrayfields['cp.fk_validator']['checked'])) {
		if ($user->rights->holiday->readall) {
			...
			$disabled=0;
			if($user_is_admin){
				$search_valideur=$user_id;
				$disabled=1;
			}
			print $form->select_dolusers($search_valideur, "search_valideur", 1, "", $disabled, $valideurarray, '', 0, 0, 0, $morefilter, 0, '', 'maxwidth150');
			print '</td>';
		} else {
