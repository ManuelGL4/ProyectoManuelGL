	...
	&& ($action == 'add' || $action == 'update') && $user->rights->societe->creer) {
		...
		if (!$error) {
			if ($action == 'add') {
				...
				if(GETPOST('options_cuentas_bank', 'alphanohtml') )
				{
					$nibans = (GETPOST('options_cuentas_bank', 'alphanohtml'));

					$arrayIbans=explode(" ",$nibans);

					foreach ($arrayIbans as $key => $iban) {
						$correctBics .= CalculoBic($iban);
						$correctBics .= " ";
					}
					
					//Guardar atributo BIC para crear usuario
					$object->array_options["options_bics"]=$correctBics;
				}
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create') {
		...
		//CUSTOM Get avg grades
		$sqlGrades="SELECT AVG(CFE.puntuacion_articulo) avgScoreArticle,AVG(CFE.puntuacion_plazos) avgScoreTime FROM ".MAIN_DB_PREFIX."commande_fournisseur_extrafields CFE, ".MAIN_DB_PREFIX."commande_fournisseur CF WHERE CFE.fk_object=CF.rowid AND  CF.fk_soc=".$socid;
		$resultGrades=$db->query($sqlGrades);
		$dataGrades=$db->fetch_object($resultGrades);
		
		$gradeTime=number_format($dataGrades->avgScoreTime,2);
		$gradeArticle=number_format($dataGrades->avgScoreArticle,2);

		print "<tr>
			<td>
				<label>Nota media tiempo</label>
			</td>
			<td>
				<input type='number' readonly value='".$gradeTime."' step='0.01'>
			</td>
		</tr>";
		print "<tr>
			<td>
				<label>Nota media calidad</label>
			</td>
			<td>
				<input type='number' readonly value='".$gradeArticle."' step='0.01'>
			</td>
		</tr>";
