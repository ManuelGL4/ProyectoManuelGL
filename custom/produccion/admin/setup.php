...
/**
 * \file    produccion/admin/setup.php
 * \ingroup produccion
 * \brief   Produccion setup page.
 */
...
require_once '../lib/produccion.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "produccion@produccion"));
...
$arrayofparameters = array(
	'PRODUCCION_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
	'PRODUCCION_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
	//'PRODUCCION_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
	//'PRODUCCION_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
	//'PRODUCCION_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
	//'PRODUCCION_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'PRODUCCION_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'PRODUCCION_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/produccion/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'PRODUCCION_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'PRODUCCION_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = produccionAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "produccion@produccion");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("ProduccionSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'PRODUCCION_'.strtoupper($myTmpObjectKey).'_ADDON';
