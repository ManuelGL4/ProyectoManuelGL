...
$arrayfields = array(
	...
	'a.location'=>array('label'=>"Location", 'checked'=>1),//Location
...
if ($search_location != '') {
	$param .= '&search_location='.urlencode($search_location);
}
...
if ($search_location) {
	$sql .= natural_search("a.location", $search_location);
}
if ($search_type_thirdparty && $search_type_thirdparty > 0) {
...
if ($resql) {
	...
	*/
	...
	//Location
	if (!empty($arrayfields['a.location']['checked'])) {
		print '<td class="liste_titre"><input type="text" class="maxwidth50" name="search_location" value="'.$search_location.'"></td>';
	}
	// Company type
	...
	//Location
	if (!empty($arrayfields['a.location']['checked'])) {
		print_liste_field_titre($arrayfields['a.location']['label'], $_SERVER["PHP_SELF"], "a.location", $param, "", "", $sortfield, $sortorder);
	}
	if (!empty($arrayfields['owner']['checked'])) {
	...
	while ($i < min($num, $limit)) {
		...
		//Location
		if (!empty($arrayfields['a.location']['checked'])) {
			print '<td class="tdoverflowmax200" title="'.dol_escape_htmltag($actionstatic->location).'">';
			print $actionstatic->location;
			print '</td>';
		}
