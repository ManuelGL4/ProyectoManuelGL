	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				if ($object->array_options['options_dir_adicional'] != "") {
					$sqlSelect = " SELECT direccion_factura FROM ".MAIN_DB_PREFIX."delegacion ";
					$sqlSelect.= " WHERE id = ".$object->array_options['options_dir_adicional']." ";

					$resultSelect = $db->query($sqlSelect);
					$dir = $db->fetch_object($resultSelect);
				}

				$pdf->writeHTMLCell (190,3, $this->posxdesc+22, $tab_top-2, $outputlangs->convToOutputCharset($dir->direccion_factura),0,1);
				...
				if (isset($_SESSION['id_ficha'])) {
					...
					$sql = "SELECT informacion_adicional AS info FROM `khns_facture_extrafields` WHERE fk_object=$id";
					$resql = $this->db->query($sql);
					$altura_maxima_primera_pagina = $posicion_y_final + 215 - 20;
					$altura_maxima_paginas_siguientes = 2150; // Altura para las páginas siguientes
				
					// EL FOOTER EMPIEZA A PARTIR DEL 215, A PARTIR DE AHI YA NO ESCRIBIR MAS
					if ($resql) {
						$row = $this->db->fetch_object($resql);
						$contenido_completo = $row->info;
						...
						while (strlen($contenido_restante) > 0) {
							// Calcular espacio disponible para el contenido en la siguiente página
							$espacio_restante_pagina_siguiente = $altura_maxima_paginas_siguientes;
							$contenido_pagina_siguiente = substr($contenido_restante, 0, $espacio_restante_pagina_siguiente);
				
							// Actualizar el contenido restante para páginas adicionales
							$contenido_restante = substr($contenido_restante, $espacio_restante_pagina_siguiente);
							...
							// Imprimir el contenido en la siguiente página
							$pdf->MultiCell($ancho_celda, $altura_celda, $contenido_pagina_siguiente, 0, null, null, null, 10.5, 50);
							$this->_pagefoot($pdf, $object, $outputlangs, 1);
						}
					} else {
				...
				// Mostrar la zona total
				$posy = $this->drawTotalTable($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs);
				

				//NUEVO PRODUCTO
				// DE ESTA MANERA NO HACE SALTO DE LINEA
				//$pdf->Cell(10,20,'Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. No sólo sobrevivió 500 años');
				//SIN Y NI X
				//$pdf->MultiCell(100,60,'Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. No sólo sobrevivió 500 años',0,null,null,null,10.5,900);
				//$pdf->MultiCell(50,20,'En el código, encuentro mi lienzo de creatividad, donde cada línea  es un pincelazo de posibilidad. En el editor de Visual Studio Code, mis ideas cobran vida, danzando en la sinfonía de la programación. Cada error es un desafío, cada solución un triunfo en mi viaje hacia la excelencia.',0,null,null,null,10.5,65);
				...
				// $this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 1, 0, $object->multicurrency_code, $outputlangsbis);
				// $pdf->MultiCell(100,60,'Lorem Ipsum es simplemente el texto de relleno de las imprentas y archivos de texto. Lorem Ipsum ha sido el texto de relleno estándar de las industrias desde el año 1500, cuando un impresor (N. del T. persona que se dedica a la imprenta) desconocido usó una galería de textos y los mezcló de tal manera que logró hacer un libro de textos especimen. No sólo sobrevivió 500 años',0,null,null,null,10.5,50);
				// $posy = $this->drawInfoTable($pdf, $object, $bottomlasttab, $outputlangs);
				// Pagefoot
				$this->_pagefoot($pdf, $object, $outputlangs);
				if (method_exists($pdf, 'AliasNbPages')) {
