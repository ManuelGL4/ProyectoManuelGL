...
/**
 * \file    test/phpunit/ProyectosFunctionalTest.php
 * \ingroup proyectos
 ...
 *
 * @package Testproyectos
 */
class ProyectosFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modProyectos')->click();
		} else {
			// Disable the module
			$this->byHref('modProyectos')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/proyectos/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('proyectos/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/proyectos/admin/about.php');
		$this->authenticate();
		return $this->assertContains('proyectos/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('proyectoswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modProyectos_ProyectosTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modProyectos_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
