	...
	if ($action == 'confirm_send') {
		...
		if ($object->statut == Holiday::STATUS_DRAFT && $cancreate) {
			...
			if ($verif > 0) {
				...
				$mail = new CMailFile($subject, "pruebas@dess.net", "pruebas@pruebas.net", $message, array(), array(), array(), '', '', 0, 0, '', '', $trackid);
		...
		if ($object->statut == Holiday::STATUS_VALIDATED && $user->id == $object->fk_validator) {
			...
			if (!$error) {
				...
				if (!$emailTo) {
					...
					$mail = new CMailFile($subject, "pruebas@pruebas.net", "pruebas@pruebas.net", $message, array(), array(), array(), '', '', 0, 0, '', '', $trackid);
