...
//PARA SABER SI SE TRATA DE UNA REPARACIÓN O NO
$sqlRep = " SELECT * FROM ".MAIN_DB_PREFIX."mrp_mo_extrafields_new ";
$sqlRep.= " WHERE fk_mo = ".$id." AND fk_averia IS NOT NULL ";

$resultRep = $db->query($sqlRep);
$esReparacion = $db->num_rows($resultRep);
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_addconsumeline' && GETPOST('addconsumelinebutton') && $permissiontoadd) {
		...
		if ($resultline <= 0) {
			...
			$sqlToproduce.= " WHERE pro.fk_mo = ".$object->id." AND pro.role = 'toproduce' ";
	
			$resultToproduce = $db->query($sqlToproduce);
			$produce = $db->fetch_object($resultToproduce);
	
			/*$sqlToconsume = " SELECT pro.*, p.rowid as idpro, p.ref, p.label FROM " . MAIN_DB_PREFIX . "mrp_production pro";
			$sqlToconsume.= " INNER JOIN " . MAIN_DB_PREFIX . "product p ON p.rowid = pro.fk_product ";
			$sqlToconsume.= " WHERE pro.fk_mo = ".$object->id." AND pro.role = 'toconsume' ";
	
			$resultToconsume = $db->query($sqlToconsume);
			$numConsu = $db->num_rows($resultToconsume);
			$consume = $db->fetch_object($resultToconsume);*/

			$sqlProducto = " SELECT ref, label FROM " . MAIN_DB_PREFIX . "product ";
			$sqlProducto.= " WHERE rowid = ".$moline->fk_product;

			$resultProducto = $db->query($sqlProducto);
			$pro = $db->fetch_object($resultProducto);
			...
			$sqlInsert.= " VALUES ";
			$sqlInsert.= " ($object->id, 0, $moline->fk_product, 'REPARACIÓN DE: ".$produce->ref." HACIENDO USO DE: ".$pro->ref."', '".$pro->label."', $moline->qty) ";
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if (in_array($action, array('consumeorproduce', 'consumeandproduceall', 'addconsumeline'))) {
		...
		if (in_array($action, array('consumeorproduce', 'consumeandproduceall'))) {
			...
			$defaultstockmovementlabel2 = "Reparación de ".$object->ref."";
			//$defaultstockmovementcode = GETPOST('inventorycode', 'alphanohtml') ? GETPOST('inventorycode', 'alphanohtml') : $object->ref.'_'.dol_print_date(dol_now(), 'dayhourlog');
			$defaultstockmovementcode2 = "Reparación de ".$object->ref."";
			...
			if ($esReparacion > 0) {
				print '<span class="fieldrequired">'.$langs->trans("InventoryCode").':</span> <input type="text" class="minwidth200 maxwidth250" name="inventorycode" value="'.$defaultstockmovementcode2.'"> &nbsp; ';
			} else {
			...
			if ($esReparacion > 0) {
				print $langs->trans("MovementLabel").': <input type="text" class="minwidth300" name="inventorylabel" value="'.$defaultstockmovementlabel2.'"><br><br>';
			} else {
