...
if (empty($reshook)) {
	...
	if ($action == 'add' && $user->rights->projet->creer) {
		...
		if (!$error) {
			...
			//------
			// Capturar el valor del ID del presupuesto del formulario
			$id_presupuesto = GETPOST('presu', 'int');

			if ($id_presupuesto != "") {
				// Asignar el ID del presupuesto al campo personalizado del proyecto
				$object->array_options['id_presu'] = $id_presupuesto;
			}
...
if ($action == 'create' && $user->rights->projet->creer) {
	...
	print '<input type="hidden" name="presu" value="'.$presu.'">';
