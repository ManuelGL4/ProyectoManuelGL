...
/**
 * \file    test/phpunit/ProduccionFunctionalTest.php
 * \ingroup produccion
...
/**
 * Class ProduccionFunctionalTest
 *
 ...
 *
 * @package Testproduccion
 */
class ProduccionFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modProduccion')->click();
		} else {
			// Disable the module
			$this->byHref('modProduccion')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/produccion/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('produccion/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/produccion/admin/about.php');
		$this->authenticate();
		return $this->assertContains('produccion/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('produccionwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modProduccion_ProduccionTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modProduccion_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
