	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$object->fetch_optionals($fk_object,$extralabels);
				
				if (isset($_SESSION['id_ficha'])) {
					$id = $_SESSION['id_ficha'];
				
					$sql = "SELECT direccion_entrega FROM khns_expedition_extrafields WHERE fk_object = $id;";
...
$col1Width = 40; // Ancho de la primera columna
$col2Width = 150; // Ancho de la segunda columna

// Definir el contenido de las columnas
$column1Content = "Dirección de Entrega";
$column2Content = $outputlangs->convToOutputCharset($direccion_entrega);

// Definir la posición ajustada
$newPosX = 10; // Ajuste hacia la izquierda
$newPosY = $tab_top -6; // Ajuste hacia arriba

// Escribir las columnas en la celda con las posiciones ajustadas
$pdf->MultiCell($col1Width, 3, $column1Content, 1, 'L', 0, 0, $newPosX, $newPosY);
$pdf->MultiCell($col2Width, 3, $column2Content, 1, 'L', 0, 1, $newPosX + $col1Width, $newPosY);


					} else {
						$pdf->writeHTMLCell(190, 3, $this->posxdesc + 22, $tab_top - 2, "Error al obtener la dirección de entrega", 0, 1);
					}
				} else {
					$pdf->writeHTMLCell(190, 3, $this->posxdesc + 22, $tab_top - 2, "ID de ficha no definido en la sesión", 0, 1);
				}
				
				// Luego, imprime $texto en una nueva celda
				$this->printStdColumnContent($pdf, $curY, "Direccion entrega", $texto);
				
				
				// if (empty($object->lines[$i]->array_options)) {
				// 	$pdf->writeHTMLCell(190, 3, $this->posxdesc + 22, $tab_top - 2, "NO ESTA VACIO", 0, 1);
				// 	foreach ($object->lines[$i]->array_options as $extrafieldColKey => $extrafieldValue) {
				// 		if ($this->getColumnStatus($extrafieldColKey)) {
				// 			// Assuming getExtrafieldContent and printStdColumnContent are defined elsewhere
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					//DE PRECIOS
					$sqlDatos = " SELECT * FROM ".MAIN_DB_PREFIX."commandedet ";
					$sqlDatos.= " WHERE rowid = ".$object->lines[$i]->fk_origin_line." ";

					$resultDatos = $this->db->query($sqlDatos);
					$datos = $this->db->fetch_object($resultDatos);

					//IVA
					if ($this->getColumnStatus('iva')) {
						//$qtyRemaining = $object->lines[$i]->qty_asked - $object->commande->expeditions[$object->lines[$i]->fk_origin_line];
						$this->printStdColumnContent($pdf, $curY, 'iva', strtr(number_format($datos->tva_tx,2),['.' => ',', ',' => '.']).'%');
						$nexY = max($pdf->GetY(), $nexY);
					}

					//PRECIO UNIDAD
					if ($this->getColumnStatus('pu')) {
						//$qtyRemaining = $object->lines[$i]->qty_asked - $object->commande->expeditions[$object->lines[$i]->fk_origin_line];
						$this->printStdColumnContent($pdf, $curY, 'pu', strtr(number_format($datos->subprice,2),['.' => ',', ',' => '.']));
						$nexY = max($pdf->GetY(), $nexY);
					}
					...
					//DESCUENTO
					if ($this->getColumnStatus('dto')) {
						//$qtyRemaining = $object->lines[$i]->qty_asked - $object->commande->expeditions[$object->lines[$i]->fk_origin_line];
						$this->printStdColumnContent($pdf, $curY, 'dto', strtr(number_format($datos->remise_percent,2),['.' => ',', ',' => '.']).'%');
						$nexY = max($pdf->GetY(), $nexY);
					}

					//TOTAL
					if ($this->getColumnStatus('total')) {
						//$qtyRemaining = $object->lines[$i]->qty_asked - $object->commande->expeditions[$object->lines[$i]->fk_origin_line];
						$this->printStdColumnContent($pdf, $curY, 'total', strtr(number_format($datos->total_ht,2),['.' => ',', ',' => '.']));
						$nexY = max($pdf->GetY(), $nexY);
					}
					...
					$base+= $datos->total_ht;
					$iva+= $datos->total_tva;
					$total+= $datos->total_ttc;
					// Obtener las coordenadas en las que termina el texto
					$posicion_y_final = $pdf->GetY();
				}
				...
				if (isset($_SESSION['id_ficha'])) {
					...
					$sql = "SELECT observaciones FROM `khns_expedition_extrafields` where fk_object=$id;";
					$resql = $this->db->query($sql);
					$altura_maxima_primera_pagina = $posicion_y_final + 215 - 20;
					$altura_maxima_paginas_siguientes = 3400; // Altura para las páginas siguientes

					if ($resql) {
						$row = $this->db->fetch_object($resql);
						$contenido_completo = $row->observaciones;

						// Verificar si el contenido de la primera página alcanza la posición Y 215
						if ($posicion_y_final < 215) {
							// Calcular espacio restante en la primera página
							$espacio_restante_primera_pagina = $altura_maxima_primera_pagina;
							$contenido_primera_pagina = substr($contenido_completo, 0, $espacio_restante_primera_pagina);
				
							// Contenido restante
							$contenido_restante = substr($contenido_completo, $espacio_restante_primera_pagina);
				
							$ancho_celda = 96;
							$altura_celda = 20;
				
							// Añadir contenido a la primera página
							$pdf->MultiCell($ancho_celda, $altura_celda, $contenido_primera_pagina, 0, null, null, null, 10.5, $posicion_y_final + 20);
							$this->_pagefoot($pdf, $object, $outputlangs, 1);
						} else {
							$contenido_restante = $contenido_completo; // El contenido completo se imprime en la segunda página
						}

						// Si hay contenido restante para imprimir en páginas siguientes
						while (strlen($contenido_restante) > 0) {
							
							// Contenido para la siguiente página
							$contenido_siguiente_pagina = substr($contenido_restante, 0, $altura_maxima_paginas_siguientes);

							// Obtener el contenido restante para páginas adicionales
							$contenido_restante = substr($contenido_restante, $altura_maxima_paginas_siguientes);

							// Agregar una nueva página
							$pdf->AddPage();
							$total_paginas_agregadas++;
							$this->_pagehead($pdf, $object, 0, $outputlangs);
							...
							// Imprimir el contenido de la siguiente página
							$pdf->MultiCell($ancho_celda, $altura_celda, $contenido_siguiente_pagina, 0, null, null, null, 10.5, 50);
							$this->_pagefoot($pdf, $object, $outputlangs, 1);
						}

					} else {
						$pdf->Cell(0, 10, 'ERROR: ' . $this->db->error());
					}
				} else {
					$pdf->Cell(0, 10, 'ERROR: El ID de la ficha no es válido');
				}
				...
				$pdf->SetXY(-60, -31); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, $outputlangs->transnoentities("TotalHT").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transcountrynoentities("TotalHT", $mysoc->country_code) : ''), 1, 'L', 1);
				$pdf->SetXY(-60, -31); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, price($base, 0, $outputlangs), 1, 'R', 1);

				//------
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(248, 248, 248);
				$pdf->SetXY(-60, -26); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, $outputlangs->transnoentities("TotalVAT").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transcountrynoentities("TotalVAT", $mysoc->country_code) : ''), 1, 'L', 1);
				$pdf->SetXY(-60, -26); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, price($iva, 0, $outputlangs), 1, 'R', 1);

				//------
				$pdf->SetTextColor(0, 0, 60);
				$pdf->SetFillColor(224, 224, 224);
				$pdf->SetXY(-60, -21); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, $outputlangs->transnoentities("TotalTTC").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transcountrynoentities("TotalTTC", $mysoc->country_code) : ''), 1, 'L', 1);
				$pdf->SetXY(-60, -21); // Ajusta estos valores según sea necesario
				$pdf->MultiCell(50, 5, price($total, 0, $outputlangs), 1, 'R', 1);
	...
	protected function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		...
		// $remaining_space_below = $pdf->getPageHeight() - $pdf->GetY() - 10;
		// if ($remaining_space_below > $default_font_size) {
		// 	$tab_height += $remaining_space_below;
		// }
