...
class pdf_crabe extends ModelePDFFactures
{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if ($object->ref_client) {
					...
					$textref .= $outputlangs->transnoentities("RefCustomer")." : ".$outputlangs->convToOutputCharset($object->ref_client);
				}
				...
				$pdf->writeHTMLCell(50, 3, $this->posxdesc + 100, $tab_top-5 , $title." : ".dol_print_date($object->date, "day", false, $outputlangs, true), 0, 1);

				if (!empty($conf->global->INVOICE_POINTOFTAX_DATE)) {
					$posy += 4;
					$pdf->SetXY($posx, $posy);
					$pdf->SetTextColor(0, 0, 60);
					$pdf->writeHTMLCell(50, 3, $this->posxdesc + 100, $tab_top , $outputlangs->transnoentities("DatePointOfTax")." : ".dol_print_date($object->date_pointoftax, "day", false, $outputlangs, true), 0, 1);
				...
				if ($object->type != 2) {
					...
					$pdf->writeHTMLCell(50, 3, $this->posxdesc + 100, $tab_top , $title." : ".dol_print_date($object->date_lim_reglement), 0, 1);
				}
				...
				if ($object->thirdparty->code_client) {
					...
					$pdf->SetXY($posx, $posy);
					$pdf->SetTextColor(0, 0, 60);
					$pdf->writeHTMLCell(50, 3, $this->posxdesc - 1, $tab_top - 5, $outputlangs->transnoentities("CustomerCode")." : ".$outputlangs->transnoentities($object->thirdparty->code_client), 0, 1);
					...
					$pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top - 19, $textref, 0, 1);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		//AQUI EMPIEZA LA REFERENCIA A LA FACTURA
		...
		if ($showaddress) {
			...
			$pdf->SetXY($posx + 50, $posy-10);
			$pdf->SetFont('', '', $default_font_size - 1);
			$pdf->MultiCell($w + 100, 4, $carac_emetteur, 0, $ltrdirection,false);
		}

		// $textref = $outputlangs->transnoentities("Ref")." : PRUEBA".$outputlangs->convToOutputCharset($object->ref);
		// if ($object->statut == $object::STATUS_DRAFT) {
		// 	$pdf->SetTextColor(128, 0, 0);
		// 	$textref .= ' - '.$outputlangs->transnoentities("NotValidated");
		// }
		...
		if ($showaddress) {
			...
			$pdf->MultiCell($widthrecbox - 2, 4, "N.I.F."." : ".$outputlangs->transnoentities($object->thirdparty->idprof1),0, $ltrdirection);
			$pdf->SetXY($posx+2 , $posy + 20);
			//NUMERO PROVEEDOR
			$pdf->MultiCell($widthrecbox - 2, 4, "Nº PROVEEDOR"." : ".$outputlangs->transnoentities($object->thirdparty->code_client),0, $ltrdirection);
			...
			$pdf->MultiCell($widthrecbox - 2, 5, "Informacion delegacion", 0, $ltrdirection);
			...
			if (isset($_SESSION['id_ficha'])) {
				...
				//DELEGACIONES
				$sqlDelegaciones = "SELECT codigo_delegacion,direccion,cp,localidad,provincia,id,nombre,responsible_name,telef1,telef2 FROM ".MAIN_DB_PREFIX."delegacion ";
				$sqlDelegaciones.= "WHERE fk_tercero = $object->socid;";

				$resultDelegacion = $this->db->query($sqlDelegaciones);
				...
				$dir = $this->db->fetch_object($resultDir);

				$posx = 102;
				$posy = 45;

				while ($delegacion = $this->db->fetch_object($resultDelegacion)) {
					if ($dir->dir_adicional == $delegacion->id) {
						$texto = $delegacion->codigo_delegacion . "\n" . $delegacion->direccion . "\n" . $delegacion->cp . " - " . $delegacion->localidad . "\n" . $delegacion->provincia . "\n\n" . $delegacion->responsible_name . " - " . $delegacion->telef1 . "\n" . $delegacion->telef2;
						
						// Calcular la altura del texto
						$altoTexto = $pdf->getStringHeight($widthrecbox, $texto);
						
						// Mostrar el texto
						$pdf->MultiCell($widthrecbox, $altoTexto, $texto, 0, 'L', false, 1, $posx, $posy);
						
						// Ajustar la posición y
						$posy += $altoTexto + 5; // Ajuste vertical
					}
				}
			}
