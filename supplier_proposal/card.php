...
$equipo = GETPOST('equipo', 'int');
$idequipo = GETPOST('idequipo', 'int');
			...
			if (!$error) {
				...
				if ($id > 0) {
					if (!$error) {
						...
						if (isset($_POST['averiafinalequipo']) && ($_POST['averiafinalequipo'] != "")) {
							$idequipo = $_POST['averiafinalequipo'];
							header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id.'&idequipo='.$idequipo.'');
							exit();
						} else {
...
if ($action == 'create') {
	...
	print '<input type="hidden" name="averiafinalequipo" value="'.$equipo.'">';
	...
	if ($idequipo != "") {
		if (empty($object->lines)) {
			...
			//SACAMOS TODOS LOS DATOS DE PRODUCTOS DEL EQUIPO
			$sqlEquipo = " SELECT p.ref, p.label, p.cost_price, p.description, ae.qty FROM ".MAIN_DB_PREFIX."product p ";
			$sqlEquipo.= " INNER JOIN ".MAIN_DB_PREFIX."averiasreparaciones_averias_equipos ae ON ae.codigo = p.rowid ";
			$sqlEquipo.= " WHERE ae.rowid = ".$idequipo;

			$resultEquipo = $db->query($sqlEquipo);
			
			while ($equipo = $db->fetch_object($resultEquipo)) {
				$desc = $equipo->description;
				$label = $equipo->ref." - ".$equipo->label;
				$precio = floatval($equipo->cost_price);
				$precio = strtr(number_format($precio,2),['.' => ',', ',' => '.']);
				$cantidad = floatval($equipo->qty);
