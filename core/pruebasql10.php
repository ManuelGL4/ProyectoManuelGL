<?php
    ...
    $ruta = "archivosImportacion/FORMA PAGO.xlsx";
    ...
    for ($fila = 2; $fila <= $filamasalta; $fila++) {
        ...
        for ($col = 1; $col <= $columnumero; $col++) {
            ...
            if ($col == 1) {    //ID
                ...
                $datos[] = $celda;
            } else if ($col == 3) {    //FECHA DESDE
                $celda = $sheet->getCellByColumnAndRow($col, $fila)->getValue();

                $datos[] = $celda;
            } else if ($col == 4) {    //USUARIO
                $celda = $sheet->getCellByColumnAndRow($col, $fila)->getValue();
        ...
        $contenido.= "INSERT INTO khns_formas_pago ";
        $contenido.= "(rowid, id_forma_pago, nombre, fecha_desde, usuario, fecha) ";
        $contenido.= "VALUES ";
        $contenido.= "(".$datos[0].", ".$datos[0].", '".$datos[1]."', '".$datos[2]."', '".$datos[3]."', '".$datos[4]."')// ";
        $contenido.= "\r\n";
        $contenido.= "\r\n";
    ...
    $contenido.= "DELIMITER ;";
    $contenido.= "\r\n";
    $contenido.= "REPRES: ".$numFormas.";";

    //die;

    // Enviar encabezados HTTP para indicar que se va a enviar un archivo para descargar
    header('Content-Type: text/plain');
    header('Content-Disposition: attachment; filename="formas_pago.sql"');
