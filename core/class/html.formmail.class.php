			...
			if (!empty($this->withfile)) {
				...
				if (is_numeric($this->withfile)) {
					...
					if ($this->withfile == 2) {
						...
						if (strpos($_SERVER["REQUEST_URI"],"compta/facture")!=false) {
							$out.=' ';
							if ($_GET["facid"]) {
								$id=$_GET["facid"];
							}else{
							...
							$out.='<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="../../compta/facture/generarFacturaElectronica.php?facid='.$id.'">Generar XML FACTURA-E</a>';
						}
					}
				} else {
					$out .= $this->withfile;
