...
$fk_report = GETPOST('fk_report', 'int');
...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
		...
		if ((GETPOST('type') == Facture::TYPE_STANDARD || GETPOST('type') == Facture::TYPE_DEPOSIT || GETPOST('type') == Facture::TYPE_PROFORMA || (GETPOST('type') == Facture::TYPE_SITUATION && !GETPOST('situations'))) && GETPOST('fac_rec') <= 0) {
			...
			if (!$error) {
				...
				// ------------------------------------------------------------------------
				// PARA COLOCAR EN LAS NUEVAS FACTURAS LOS VALORES POR DEFECTO DEL CLIENTE
				// EN LOS CAMPOS DE CONDICIONES DE PAGO Y MODO DE PAGO
				// CUSTOM
				// SACAMOS LA ID DE LAS CONDICIONES DEL PAGO
				$sql = "SELECT cond_reglement ";
				$sql.= "FROM ".MAIN_DB_PREFIX."societe ";
				$sql.= "WHERE rowid = ".$object->socid.";";

				$result = $db->query($sql);
				$condi = $db->fetch_object($result);
				$condiFinal = $condi->cond_reglement;
				$object->cond_reglement_id = $condiFinal;

				// SACAMOS LA ID DEL MODO DE PAGO
				$sql = "SELECT mode_reglement ";
				$sql.= "FROM ".MAIN_DB_PREFIX."societe ";
				$sql.= "WHERE rowid = ".$object->socid.";";
	
				$result = $db->query($sql);
				$modo = $db->fetch_object($result);
				$modoFinal = $modo->mode_reglement;
				$object->mode_reglement_id = $modoFinal;

				// SACAMOS LA CUENTA BANCARIA
				$sql = "SELECT fk_account ";
				$sql.= "FROM ".MAIN_DB_PREFIX."societe ";
				$sql.= "WHERE rowid = ".$object->socid.";";
	
				$result = $db->query($sql);
				$cuentaB = $db->fetch_object($result);
				$cuentaBFinal = $cuentaB->fk_account;
				$object->fk_account = $cuentaBFinal;
				...
				$result = $db->query($sql);
				$libre_1 = $db->fetch_object($result);
				$libre_1 = $libre_1->libre_1;

				//Esto es para sacar el texto adicional, en función de las condiciones del pago
				/*if ($object->cond_reglement_id != "") {
					$sqlTexto = " SELECT texto ";
					$sqlTexto.= " FROM ".MAIN_DB_PREFIX."c_payment_term_text ";
					$sqlTexto.= " WHERE fk_cond = ".$object->cond_reglement_id;

					$resultSqlTexto = $db->query($sqlTexto);

					$textoFinal = $db->fetch_object($resultSqlTexto);

					$object->array_options['options_cond_text'] = $textoFinal->texto;
				}*/
				if ($libre_1 != "") {

					$object->array_options['options_cond_text'] = $libre_1;
				}
				// ------------------------------------------------------------------------

				if ($_POST["options_fk_report"] != -1) {

					//Fill the product lines with equipment of the contract related to the selected project
					//$object->fac_rec = "";

					//TODO check spare (repuestos) to change the prices if necesary
					...
					$sqlEquipment = " SELECT p.description, mce.location, p.tva_tx, p.ref, p.label, pom.taxable_base, pom.fk_product, pom.quantity, pom.price as subprice, pom.discount, d.iva, ";
					$sqlEquipment.= " (".$sqlTotalHt.") as total_ht, ( ".$sqlTotalTva." ) as total_tva,   ";
					$sqlEquipment.= " ( (".$sqlTotalHt.") + (".$sqlTotalTva.")  ) as total_ttc,   ";
					$sqlEquipment.= " mc.contact_discount as dto_oferta, i.contract_id, mc.client_discount as dto_cliente, mc.spare_parts_discount ";
					$sqlEquipment.= " FROM ".MAIN_DB_PREFIX."mantenimiento_contratos_equipos mce";
					$sqlEquipment.= " INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos mc ON mc.rowid = mce.fk_contract";
					$sqlEquipment.= " INNER JOIN ".MAIN_DB_PREFIX."delegacion d ON d.id = mc.delegation_id";
					$sqlEquipment.= " INNER JOIN ".MAIN_DB_PREFIX."proyectos_oferta_materiales pom ON pom.fk_project = mc.project_id";
					$sqlEquipment.= " INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_informes i ON i.contract_id = mc.rowid";
					$sqlEquipment.= " INNER JOIN ".MAIN_DB_PREFIX."product p ON p.rowid = pom.fk_product";
					$sqlEquipment.= " WHERE i.rowid = ".$_POST["options_fk_report"];
					$sqlEquipment.= " GROUP BY pom.fk_product";
					
					$resultEquipment = $db->query($sqlEquipment);

					$sqlLocations = " SELECT mce.location FROM khns_mantenimiento_contratos mc ";
					$sqlLocations.= " INNER JOIN khns_mantenimiento_informes mi ON mc.rowid = mi.contract_id ";
					$sqlLocations.= " INNER JOIN khns_mantenimiento_contratos_equipos mce ON mce.fk_contract = mc.rowid ";
					$sqlLocations.= " WHERE mi.rowid = ".$_POST["options_fk_report"];
					...
					$simantImpIva = 0;
					$simantPrecioFinal = 0;
					$simantDesc = "SIMANT - Trabajos Preventivos. Cuota de mantenimiento de conservación del siguiente mantenimiento: \n";

					while ($producto = $db->fetch_object($resultEquipment)) {
					...
					$sqlCosteCero = " SELECT p.rowid as idpro, p.ref, p.description, p.label, mcr.quantity, mcr.rowid, mcr.mantenido ";
					$sqlCosteCero.= "FROM ".MAIN_DB_PREFIX."mantenimiento_informes mi ";
					$sqlCosteCero.= "INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos mc ON mi.contract_id = mc.rowid ";
					$sqlCosteCero.= "INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos_repuestos mcr ON mcr.fk_contract = mc.rowid ";
					$sqlCosteCero.= "INNER JOIN ".MAIN_DB_PREFIX."product p ON p.rowid = mcr.fk_product ";
					$sqlCosteCero.= "WHERE mi.rowid = ".$_POST["options_fk_report"];

					$resultCosteCero = $db->query($sqlCosteCero);


					$sqlDele = " SELECT de.iva ";
					$sqlDele.= "FROM ".MAIN_DB_PREFIX."mantenimiento_informes mi ";
					$sqlDele.= "INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos mc ON mi.contract_id = mc.rowid ";
					$sqlDele.= "INNER JOIN ".MAIN_DB_PREFIX."delegacion de ON de.id = mc.delegation_id ";
					$sqlDele.= "WHERE mi.rowid = ".$_POST["options_fk_report"];

					$resultDele = $db->query($sqlDele);
					$dele = $db->fetch_object($resultDele);

					// SIMANT ----------------------------- (revisar)
					/*$sqlSimant = "SELECT rowid, ref, label, , tva_tx, description, price,  FROM ".MAIN_DB_PREFIX."product ";
					$sqlSimant.= "WHERE ref LIKE '%SIMANT%' ";

					$resultSimant = $db->query($sqlSimant);
					$simant = $db->fetch_object($resultSimant);*/

					$line = new FactureLigne($db);
					//$line->fk_product = $simant->rowid;
					$line->subprice = $simantSubprice;
					//print "SUBPRICE ".$line->subprice;
					//print "<br>";
					$line->qty = 1;
					//print "CANTIDAD ".$line->qty;
					//print "<br>";
					$line->desc = $simantDesc;
					//print "DESC ".$line->desc;
					//print "<br>";
					$line->remise_percent = $simantDescuento;
					//print "REMISE ".$line->remise_percent;
					//print "<br>";
			
					$line->product_type = 1;
					//base imponible total
					if ($line->remise_percent > 0) {
						$descuento = ($line->subprice * $line->remise_percent) / 100;
					} elseif ($line->remise_percent == 0) {
						$descuento = 0;
					}

					if ($descuento > 0) {
						$line->total_ht = $line->subprice - $descuento;
					} else {
						$line->total_ht = $line->subprice;
					}

					//iva total
					$line->total_tva = ($line->total_ht / 100) * $dele->iva;
					//print "TOTAL TVA ".$line->total_tva;
					//print "<br>";
					//importe total
					$line->total_ttc = $line->total_ht + $line->total_tva;
					//print "IMP. TOTAL ".$line->total_ttc;
					//print "<br>";
					$line->tva_tx = $dele->iva;
					//print "IVA ".$line->tva_tx;
					//print "<br>";

					//$line->localtax1_tx = $equipment->description;
					$line->localtax1_type = 3;
					//$line->localtax2_tx = $equipment->description;
					$line->localtax2_type = 1;

					array_push($object->lines, $line);
					// SIMANT -----------------------------
				

					if(!$resultCosteCero) $error++;

					// Add product individual
					while ($costeCero = $db->fetch_object($resultCosteCero)) {
						
						//A coste cero porque está mantenido
						if (($costeCero->mantenido == 1)) {

							$line = new FactureLigne($db);
							$line->fk_product = $costeCero->idpro;
							//$line->element = $equipment->fk_product;
							$line->subprice = 0;
							$line->qty = $costeCero->quantity;
							$line->desc = $costeCero->description;
						...
						} else if (($costeCero->mantenido == 0) || ($costeCero->mantenido == "")) {

							//SACAMOS PRECIO PRODUCTO
							$sqlPrecio = " SELECT price FROM ".MAIN_DB_PREFIX."product p ";
							$sqlPrecio.= " WHERE rowid = ".$costeCero->idpro." ";
							...
							$base = ($precioFinal * $costeCero->quantity) - $descontado;
							$line->total_ht = $base;
							//iva total
				...
				if (!empty($origin) && !empty($originid)) {
					...
					//Para comprobar si puede crearse una nueva factura o no

					/*print $_POST['options_fk_report'];
					print '<br>';
					print $_POST['options_factura_adicional'];
					print '<br>';
					print $object->date  ;
					print '<br>';*/
					$fecha = date("Y-m-d", $object->date  );
					//print $fecha;
					list($ano, $mes, $dia) = explode("-", $fecha);
					/*print '<br>';
					print $mes;
					print '<br>';*/
					$informe = $_POST['options_fk_report'];
					$facturaA = $_POST['options_factura_adicional'];

					$contrato = "SELECT mi.contract_id FROM ".MAIN_DB_PREFIX."mantenimiento_informes mi INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos mc ON mc.rowid = mi.contract_id WHERE mi.rowid = ".$informe;
					//print $contrato;
					$resultContrato = $db->query($contrato);
					$contrato = $db->fetch_object($resultContrato);
					
					$consulta2 = "SELECT DISTINCT mc.rowid, f.rowid, f.datef, fe.factura_adicional FROM ".MAIN_DB_PREFIX."facture f INNER JOIN ".MAIN_DB_PREFIX."facturedet fd ON f.rowid = fd.fk_facture INNER JOIN ".MAIN_DB_PREFIX."facture_extrafields fe ON fe.fk_object = f.rowid INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_informes mi ON fe.fk_report = mi.rowid INNER JOIN ".MAIN_DB_PREFIX."mantenimiento_contratos mc ON mc.rowid = mi.contract_id WHERE mc.rowid = ".$contrato->contract_id;

					/*print '<br>';
					print $contrato->contract_id;
					print '<br>';
					print $consulta2;
					print '<br>';*/

					$resultConsulta2 = $db->query($consulta2);
					$filasConsulta2 = $db->num_rows($consulta2);

					$valida = 1;

					/*if ($filasConsulta2 > 0) {
						$lista = array();
						while ($resultado = $db->fetch_object($resultConsulta2)) {
							//print $resultado->datef;
							//print '<br>';
							list($ano2, $mes2, $dia2) = explode("-", $resultado->datef);
							$lista[] = $resultado->factura_adicional;
							$lista[] = $ano;
							$lista[] = $mes;

							if ($resultado->factura_adicional == 0) {
								if ($ano == $ano2 && $mes == $mes2) {
									//print "mes 1:".$mes." mes2:".$mes2;
									//print "ERROR, YA HAY UNA FACTURA PARA ESE CONTRATO EN ESE MES";
									$valida = 0;
									setEventMessages("No se puede facturar en esa fecha para ese informe", $object->errors, 'errors');
									break;
								}
							} else {
					...
					/*print '<br>';
					print "ES VALIDA????: ".$valida;
					print '<br>';

					print var_dump($lista);*/
	...
	/*if (isset($_POST['attribute'])) {
		if (isset($_POST['options_cond_text'])) {
			$texto = $_POST['options_cond_text'];

			//Comprobamos si ya hay un texto para esas condiciones de pago
			$sql = "SELECT * FROM ".MAIN_DB_PREFIX."c_payment_term_text WHERE fk_cond = ".$object->cond_reglement_id;
			$result = $db->query($sql);
			$numFilas = $db->num_rows($result);

			if ($numFilas > 0) {
				$sqlUpdate = "UPDATE ".MAIN_DB_PREFIX."c_payment_term_text SET texto = '".$texto."' WHERE fk_cond = ".$object->cond_reglement_id;
				$db->query($sqlUpdate);
			} else {
				$sqlInsert = "INSERT INTO ".MAIN_DB_PREFIX."c_payment_term_text (fk_cond, texto) VALUES ($object->cond_reglement_id, '".$texto."')";
				$db->query($sqlInsert);
			}
	...
	if (isset($_POST['attribute'])) {
		if (isset($_POST['options_cond_text'])) {
			...
			$sqlUpdate = "UPDATE ".MAIN_DB_PREFIX."societe_extrafields SET libre_1 = '".$texto."' WHERE fk_object = ".$object->socid;
			$db->query($sqlUpdate);
...
if ($action == 'create') {
	...
	if (empty($reshook)) {
		...
		if ($fk_report != "") {
			$object->array_options['options_fk_report'] = $fk_report;
		}
	...
	if($id!=null){
		...
		$sqlEmail="SELECT S.email FROM ".MAIN_DB_PREFIX."societe S,".MAIN_DB_PREFIX."facture F WHERE F.fk_soc=S.rowid and F.rowid=".$id;
		$resultEmail=$db->query($sqlEmail);
		$dataEmail=$db->fetch_object($resultEmail);
		

		print '<script type="text/javascript" language="javascript">
			$(document).ready(function() {
				var email=document.getElementById("sendto");
				email.value="'.$dataEmail->email.'";
			});
		</script>';
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->statut == Facture::STATUS_VALIDATED) {
				//Generar XML factura electrónica
				//if(isset($object->id)){
					print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="../facture/generarFacturaElectronica.php?facid='.$object->id.'">Generar FE</a>';
					//print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="facegenerar.php?facid='.$object->id.'">Generar FE</a>';
					
					if ($object->array_options['options_fk_report'] != "") {
						print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="../facture/printSimath.php?facid='.$object->id.'">Generar SIMANT</a>';
					}
					//send invoice facturae
					print '<a class="butAction'.($conf->use_javascript_ajax ? ' reposition' : '').'" href="'.$_SERVER['PHP_SELF'].'?action=facturae&facid='.$object->id.'">Enviar FE</a>';
				//}
			...
			if ($_GET["message"] != "") {
				setEventMessages($_GET["message"], '', 'mesgs');
			}
			//send invoice facturae form
			if ($_GET["action"] == "facturae") {
			
				print '
				<form method="POST" action="sendInvoiceB2B.php" name="formfilter" autocomplete="off" enctype="multipart/form-data">
					<div tabindex="-1" role="dialog" class="ui-dialog ui-corner-all ui-widget ui-widget-content ui-front ui-dialog-buttons ui-draggable" aria-describedby="dialog-confirm" aria-labelledby="ui-id-1" style="height: auto; width: 500px; top: 268.503px; left: 457.62px; z-index: 101;">
						<div class="ui-dialog-titlebar ui-corner-all ui-widget-header ui-helper-clearfix ui-draggable-handle">
							<span id="ui-id-1" class="ui-dialog-title">Enviar factura electrónica</span>
							<a href="'.$_SERVER['PHP_SELF'].'?facid='.$id.'" class="ui-button ui-corner-all ui-widget ui-button-icon-only ui-dialog-titlebar-close">
						...
						<div id="dialog-confirm" style="width: auto; min-height: 0px; max-height: none; height: 97.928px;" class="ui-dialog-content ui-widget-content">
							...
							<div class="">
								<table>
									<tr>
										<td>
											<span class="field">Factura</span>
										</td>
										<td>
											<input type="hidden" name="facid" value="'.$id.'">
											<input required type="file" name="invoice" accept=".xml">
										</td>
									</tr>
								</table>
							</div>
						...
						<div class="ui-dialog-buttonpane ui-widget-content ui-helper-clearfix">
							<div class="ui-dialog-buttonset">
								...
								<a href="'.$_SERVER['PHP_SELF'].'?facid='.$id.'" class="ui-button ui-corner-all ui-widget">No</a>
							</div>
						</div>
					</div>
				</form>
...
if ($action == "create") {
	echo '<script>
		...
		var elemento = document.getElementsByClassName("facture_extras_cond_text")[0];
