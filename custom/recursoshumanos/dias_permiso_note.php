...
/**
 *  \file       dias_permiso_note.php
 *  \ingroup    recursoshumanos
 *  \brief      Tab for notes on Dias_permiso
 */
...
$langs->loadLangs(array("recursoshumanos@recursoshumanos", "companies"));
...
$hookmanager->initHooks(array('dias_permisonote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->recursoshumanos->multidir_output[$object->entity]."/".$object->id;
}

$permissionnote = $user->rights->recursoshumanos->dias_permiso->write; // Used by the include of actions_setnotes.inc.php
$permissiontoadd = $user->rights->recursoshumanos->dias_permiso->write; // Used by the include of actions_addupdatedelete.inc.php
...
$help_url = '';
llxHeader('', $langs->trans('Dias_permiso'), $help_url);
