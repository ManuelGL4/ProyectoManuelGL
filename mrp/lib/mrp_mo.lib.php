...
function moPrepareHead($object)
{
	...
	$sqlComprobacion = " SELECT rowid FROM ".MAIN_DB_PREFIX."mrp_mo_extrafields_new ";
	$sqlComprobacion.= " WHERE fk_mo = ".$object->id;

	$resultComprobacion = $db->query($sqlComprobacion);
	$comprobacion = $db->fetch_object($resultComprobacion);
	$comprobacion = $comprobacion->rowid;

	//if ($comprobacion != "") {
		$head[$h][0] = DOL_URL_ROOT.'/mrp/mo_lineas.php?id='.$object->id;
		$head[$h][1] = $langs->trans("Líneas y Costes");
		$head[$h][2] = 'lineas';
		$h++;

		$head[$h][0] = DOL_URL_ROOT.'/mrp/mo_trabajadores.php?id='.$object->id;
