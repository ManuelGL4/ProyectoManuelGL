...
if(empty($search_name) && $user_is_admin==0){
	$search_name = $user->firstname.' '.$user->lastname;
}
...
// Adjust the filter if the user is an administrator
if ($user_is_admin==1) {
    $filters = ' AND u.fk_user = ' . $user->id;
} 
...
if (count($typeleaves) == 0) {
	...
	// User
	print '<td class="liste_titre"><input type="text" name="search_name"';
	if ($user_is_admin == 0) {
		print ' disabled';
	}
	print ' value="' . dol_escape_htmltag($search_name) . '"></td>';
	...
	if($user_is_admin==1){
		$disabled=1;
		$user_id = $user->id;
		$search_supervisor=$user_id;
	}
	print $form->select_dolusers($search_supervisor, 'search_supervisor', 1, null, $disabled, null, null, 0, 0, 0, '', 0, '', 'maxwidth200');
	print '</td>';
