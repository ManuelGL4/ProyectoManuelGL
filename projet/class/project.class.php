...
class Project extends CommonObject
{
	...
	public function create($user, $notrigger = 0)
	{
		...
		if (!$error) {
			...
			//RET ES EL ID DEL PROYECTO CREADO
			//HAY QUE ACTUALIZAR EL PROYECTO EN EL PRESUPUESTO Y CARGAR LOS MATERIALES AL PROYECTO
			if ($ret > 0) {

				$presup = $this->array_options['id_presu'];

				if ($presup != "") {
					$sqlUpdate = " UPDATE ".MAIN_DB_PREFIX."propal ";
					$sqlUpdate.= " SET fk_projet = ".$ret." ";
					$sqlUpdate.= " WHERE rowid = ".$presup." ";

					$this->db->query($sqlUpdate);

					$sqlPresu = " SELECT p.*, pd.* FROM ".MAIN_DB_PREFIX."propal p ";
					$sqlPresu.= " INNER JOIN ".MAIN_DB_PREFIX."propaldet pd ON pd.fk_propal = p.rowid ";
					$sqlPresu.= " WHERE p.rowid = ".$presup." ";

					$resultPresu = $this->db->query($sqlPresu);
					
					while ($presu = $this->db->fetch_object($resultPresu)) {

						$total =  $presu->qty * $presu->subprice;

						$sqlInsert = " INSERT INTO ".MAIN_DB_PREFIX."proyectos_oferta_materiales ";
						$sqlInsert.= " (fk_project, fk_product, fk_chapter, quantity, price, discount, taxable_base, material_cost, transport_cost, installation_cost, development_cost, test_cost, equivalence) ";
						$sqlInsert.= " VALUES ";
						$sqlInsert.= " ($ret, $presu->fk_product, 1, $presu->qty, $presu->subprice, 0, $total, 0, 0, 0, 0, 0, 0) ";
	...
	public function delete($user, $notrigger = 0)
	{
		...
		//BORRAMOS MATERIALES
		$sqlBorrado = " DELETE FROM ".MAIN_DB_PREFIX."proyectos_oferta_materiales ";
		$sqlBorrado.= " WHERE fk_project = ".$this->id." ";

		$this->db->query($sqlBorrado);
