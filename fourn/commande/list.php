...
if (empty($reshook)) {
	...
	if ($massaction == 'confirm_createsupplierbills') {
		...
		foreach ($orders as $id_order) {
			...
			// echo "                                                                        Valor de \$id_order: ";
			// print_r($id_order);
			// echo "\n";
			// echo "                                                                        VALOR DE OBJECT ID: ";
			// print_r($objecttmp->id);
			// echo "\n";
			// echo "                                                                        VALOR DE ORIGIN: ";
			// print_r($objecttmp->origin);
			// echo "\n";
			 $objecttmp->ref_supplier='prueba';
			// echo "                                                                        VALOR DE ELEMENT: ";
			// print_r($objecttmp->ref_supplier);
			// echo "\n";
			if ($objecttmp->id > 0) {
		...
		if (!$error) {
			...
			if ($nb_bills_created == 1) {
				...
				//DELTANET $cmd->socid=2
				//Último error en solicitud de acceso de base de datos: UPDATE khns_commande_fournisseur SET billed = 1 WHERE rowid = AND fk_statut > 0
				//$id_order=23
