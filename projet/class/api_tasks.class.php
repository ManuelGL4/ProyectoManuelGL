 ...
 */
 ...
 require_once DOL_DOCUMENT_ROOT.'/projet/class/custom/TimesheetAttendanceEvent.class.php';
...
class Tasks extends DolibarrApi
{
	...
	/**
	 * @var Attendanceevent $event {@type Attendanceevent}
	 */
	public $event;
	...
	public function __construct()
	{
		...
		$this->event=new Attendanceevent($this->db,DolibarrApiAccess::$user->id);
	}
	...
	/**
	 *
	 * Return an array with all tasks informations
	 *
	 * @return 	array(string)     data without useless information
	 * @url GET events
	 * 
	...
	{
		if (!DolibarrApiAccess::$user->rights->timesheet->attendance->user) {
			throw new RestException(401, 'Not enought permissions');
		}

		$taskList=$this->event->fetchTasks();
	...
	public function getLastTask()
	{
		...
		$success=$this->event->fetch('',DolibarrApiAccess::$user);
	 ...
	 * @return  int     ID of task event
	 * @url POST events/start
	 */
	public function postEventStart($request_data = null)
	{
		if (!DolibarrApiAccess::$user->rights->timesheet->attendance->user) {
		...
		// if ($this->event->ajaxStart(DolibarrApiAccess::$user) < 0) {
		// 	throw new RestException(500, "Error creating task", array_merge(array($this->task->error), $this->task->errors));
		// }
		
		$customer=$request_data['third_party'];
		$project=$request_data['project'];
		$task=$request_data['task'];
		$this->event->event_location_ref=$request_data['event_location_ref'];
		$this->event->note=$request_data['note'];

		try {

			$this->event->ajaxStart(DolibarrApiAccess::$user,'',$customer,$project,$task);

		} catch (\Throwable $th) {
			throw new RestException(500,"Error starting task");
		}
	 ...
	 * @param   array   $request_data   Request data
	 * @return  int     ID of task event
	 * @url POST events/stop
	 */
	public function postEventStop($request_data = null)
	{
		if (!DolibarrApiAccess::$user->rights->timesheet->attendance->user) {
			throw new RestException(401, "Insuffisant rights");
		}
		...
		if ($this->event->ajaxStop(DolibarrApiAccess::$user) < 0) {
			throw new RestException(500, "Error stoping task", array_merge(array($this->task->error), $this->task->errors));
		}
