			...
			if (!$error) {
				...
				if ($id > 0) {
					...
					if (!$error) {
						...
						if (isset($_POST['averiafinal']) && ($_POST['averiafinal'] != "")) {
							$idaveria = $_POST['averiafinal'];
							header('Location: '.$_SERVER["PHP_SELF"].'?id='.$id.'&idaveria='.$idaveria.'');
							exit();
						} else {
...
if ($action == 'create') {
	...
	print '<input type="hidden" name="averiafinal" value="'.$idaveria.'">';
	...
	print '<input type="submit" class="button" value="'.$langs->trans("CreateDraft").'" name="creado">';
	print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	...
	if ($idaveria != "") {
		if (empty($object->lines)) {
			...
			//SACAMOS TODOS LOS DATOS DE PRODUCTOS DE LA AVERÍA
			$sqlProductos = " SELECT me.*, s.remise_client, pe.discount_offer, se.porc_iva as ivacli, a.*, mp.role, mp.qty as cantidad, mp.fk_product as producto, mp.fk_mo as idmo, mo.ref as refot, mo.label as labelot FROM ".MAIN_DB_PREFIX."mrp_mo_extrafields_new me ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."averiasreparaciones_averias a ON a.rowid = me.fk_averia ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."mrp_production mp ON mp.fk_mo = me.fk_mo ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."mrp_mo mo ON mo.rowid = mp.fk_mo ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."societe_extrafields se ON se.fk_object = a.fk_cliente ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."societe s ON s.rowid = se.fk_object ";
			$sqlProductos.= " INNER JOIN ".MAIN_DB_PREFIX."projet_extrafields pe ON pe.fk_object = a.fk_project ";
			$sqlProductos.= " WHERE me.fk_averia = ".$idaveria." AND mp.role LIKE '%toproduce%' ";

			$resultProductos = $db->query($sqlProductos);
			
			while ($pro = $db->fetch_object($resultProductos)) {
				$desc = $pro->refot."-".$pro->labelot;
				$precio = floatval($pro->gasto_actual);
				$precio = strtr(number_format($precio,2),['.' => ',', ',' => '.']);
				$cantidad = floatval($pro->cantidad);
				$producto = intval($pro->producto);
				$dto = floatval($pro->dto);
				$iva = floatval($pro->ivacli);

				$dto+= $pro->remise_client;
				$dto+= $pro->discount_offer;

				$result = $object->addline($desc, $precio, $cantidad, $iva, "", "", $producto, $dto, "HT", $pu_ttc, $info_bits, $type, - 1, 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $date_start, $date_end, $array_options, $fk_unit, '', 0, $pu_ht_devise);
				//$result = $object->addline("'.$desc.'", 25, $cantidad, 0, "", "", $producto, $dto, "", 0, "HT", "", "", "", "", "", "", "", "", "", "", "", "", '', "", "");

				$updateOferta = " UPDATE ".MAIN_DB_PREFIX."mrp_mo_extrafields_new ";
				$updateOferta.= " SET fk_oferta = ".$object->id." ";
				$updateOferta.= " WHERE fk_averia = ".$idaveria." AND fk_mo = ".$pro->idmo." ";

				$db->query($updateOferta);
			}

			//die;

			print '<meta http-equiv="refresh" content="0; url=' . $_SERVER["PHP_SELF"].'?id='.$id . '">';
			...
			//$result = $object->addline("ESTA ES LA DESC", 50, 3, 0, "", "", 7, 30, "", 0, "HT", "", "", "", "", "", "", "", "", "", "", "", "", '', "", "");
		}
	}
			...
			if ($action != 'editline') {
				...
				if ($object->statut == Propal::STATUS_SIGNED) {
					if (!empty($conf->projet->enabled)) {
						...
						//SACAMOS LOS DTO DE CLIENTE Y OFERTA POR SEPARADO
						$sqlDescuentos = " SELECT pe.descripcion, pd.remise_percent, s.remise_client FROM ".MAIN_DB_PREFIX."propaldet pd ";
						$sqlDescuentos.= " INNER JOIN ".MAIN_DB_PREFIX."propal p ON p.rowid = pd.fk_propal ";
						$sqlDescuentos.= " INNER JOIN ".MAIN_DB_PREFIX."propal_extrafields pe ON pe.fk_object = p.rowid ";
						$sqlDescuentos.= " INNER JOIN ".MAIN_DB_PREFIX."societe s ON s.rowid = p.fk_soc ";
						$sqlDescuentos.= " WHERE pd.fk_propal = ".$object->id."";

						$resultDescuentos = $db->query($sqlDescuentos);
						$descuentos = $db->fetch_object($resultDescuentos);

						$descOferta = $descuentos->remise_percent - $descuentos->remise_client;
						...
						print '<a class="butAction" href="'.DOL_URL_ROOT.'/projet/card.php?action=create&socid='.$object->socid.'&description='.$descuentos->descripcion.'&options_discount_offer='.$desc.'&title=Proyecto de oferta '.$object->ref.'&budget_amount='.strtr(number_format($object->total_ttc,2),['.' => ',', ',' => '.']).'&idpresu='.$id.'" target="_blank">'.$langs->trans("Crear proyecto").'</a>';
					}
				}
