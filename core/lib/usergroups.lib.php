...
function user_prepare_head($object)
{
	...
	if (empty($user->socid)) {
		...
		if ($numFilas > 0) {
			$head[$h][1] .= '<span class="badge marginleftonlyshort">'.$numFilas.'</span>';
		}
		...
		$consulta = "SELECT r.ref FROM ".MAIN_DB_PREFIX."element_contact ec INNER JOIN ".MAIN_DB_PREFIX."resource r ON ec.element_id = r.rowid WHERE ec.fk_c_type_contact = ".$idContact->rowid." AND ec.fk_socpeople = ".$object->id;
		$resultConsulta = $db->query($consulta);
		$numFilas = $db->num_rows($resultConsulta);

		$head[$h][0] = DOL_URL_ROOT.'/user/userresources.php?id='.$object->id;
		$head[$h][1] = $langs->trans("Recursos");
		if ($numFilas > 0) {
			$head[$h][1] .= '<span class="badge marginleftonlyshort">'.$numFilas.'</span>';
		}
		$head[$h][2] = 'recursos';
