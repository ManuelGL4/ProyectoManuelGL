		...
		if ($result >= 0) {
			...
			if ($result) {
				...
				if (!$error) {
					...
					$this->db->commit();
					$sqlDescuento = " SELECT rowid, discount FROM ".MAIN_DB_PREFIX."societe_extrafields ";
					$sqlDescuento.= " WHERE fk_object = ".$this->id;
					$resultDescuento = $this->db->query($sqlDescuento);
					$descuento = $this->db->fetch_object($resultDescuento);

					$sqlUpdate = " UPDATE ".MAIN_DB_PREFIX."societe SET remise_client = ".$descuento->discount." WHERE rowid = ".$this->id;
