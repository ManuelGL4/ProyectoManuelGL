...
class SupplierInvoices extends DolibarrApi
{
	...
	/**
	 * Add supplier invoice document
	 * 
	 * @param int $id   Supplier invoice ID
	 * @param array  $invoice   Invoice file
	 * 
	 * @url     POST /{id}/document
	 * 
	 * @return boolean                  Is successfully added
	 */
	public function addInvoiceDocument($id , $invoice)
	{
		global $conf;
		$isAdded = false;
		...
		if (($_FILES['invoice']['name'] != "")){
			...
			$upload_dir = $conf->fournisseur->facture->dir_output.'/'.get_exdir($this->invoice->id, 2, 0, 0, $this->invoice, 'invoice_supplier').$ref;
			$file = $_FILES['invoice']['name'];
			$path = pathinfo($file);
			$filename = $path['filename'];
			$ext = $path['extension'];
			$temp_name = $_FILES['invoice']['tmp_name'];
			$path_filename_ext = $upload_dir."/".$filename.".".$ext;
			// Check if file already exists
			
			if (!file_exists($upload_dir)) {
				throw new RestException(400, "Not a valid invoice");
			}

			$isAdded = move_uploaded_file($temp_name,$path_filename_ext);
		}
