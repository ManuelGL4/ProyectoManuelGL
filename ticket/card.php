...
if (empty($reshook)) {
	...
	if (GETPOST('add', 'alpha') && !empty($user->rights->ticket->write)) {
		...
		if (!$error) {
			...
			if (!$error) {
				...
				// Correo electronico de quien envia
				$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
				// Asunto y mensaje
				$subject="Nuevo Ticket";
				$message="Asunto: $object->subject\nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
				."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;

				$fuser = new User($db);
				$fuser->fetch($object->fk_user_create);
				$email_user = $fuser->email;
				// Enviar mensaje con CC al creador del ticket
				$mailfile = new CMailFile($subject, $sendto, $from, $message, null, null, null, $email_user);
	...
	if ($action == 'update' && $user->rights->ticket->write && $object->fk_statut < Ticket::STATUS_CLOSED) {
		...
		if (!$error) {
			...
			$message="El ticket nº $object->ref ha sido actualizado \nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
			."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;
	...
	if ($action == "mark_ticket_read" && $user->rights->ticket->write) {
		...
		if ($object->markAsRead($user) > 0) {
			...
			$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
			// Asunto y mensaje
			$subject="Ticket leido";
			$message="El ticket nº: $object->ref ha sido marcado como 'Leido' ";
	...
	if ($action == "assign_user" && GETPOST('btn_assign_user', 'alpha') && $user->rights->ticket->write) {
		...
		if (!$error) {    // Update list of contacts
			...
			if ($usertoassign > 0) {
				...
				$sql = "SELECT firstname, lastname FROM khns_user WHERE rowid = $object->fk_user_assign";
				$res = $db->query($sql);
				$row = $res->fetch_assoc();
				...
				$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
				// Asunto y mensaje
				$subject="Usuario asignado a ticket";
				$message="El usuario $nombre $apellidos ha sido asignado al ticket nº $object->ref \nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
				."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;
	...
	if (($action == "confirm_close" || $action == "confirm_abandon") && GETPOST('confirm', 'alpha') == 'yes' && $user->rights->ticket->write) {
		...
		if ($object->close($user, ($action == "confirm_abandon" ? 1 : 0))) {
			...
			$message="El ticket $object->ref se ha cerrado correctamente,esta era la información mas relevante\nAsunto: $object->subject\nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
			."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;
	...
	if ($action == 'set_thirdparty' && $user->rights->ticket->write) {
		if ($object->fetch(GETPOST('id', 'int'), '', GETPOST('track_id', 'alpha')) >= 0) {
			...
			$sql = "SELECT nom FROM khns_societe WHERE rowid=$object->fk_soc";
			$res = $db->query($sql);
			$row = $res->fetch_assoc();
    		$nombre = $row["nom"];
			...
			$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
			// Asunto y mensaje
			$subject="Compañia asignada a ticket";
			$message="La compañia ". $nombre." ha sido asignada al ticket nº $object->ref  \nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
			."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;
	...
	if ($action == 'confirm_reopen' && $user->rights->ticket->manage && !GETPOST('cancel')) {
		if ($object->fetch(GETPOST('id', 'int'), '', GETPOST('track_id', 'alpha')) >= 0) {
			...
			if ($object->fk_statut == Ticket::STATUS_CLOSED || $object->fk_statut == Ticket::STATUS_CANCELED) {
				...
				if ($res) {
					...
					$message="Le informamos que el ticket $object->ref ha sido reabierto,esta es la informacion del ticket\nAsunto: $object->subject\nTipo de solicitud: ".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'))
					."\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label'))."\nMensaje: ".$object->message;
		...
		if (!GETPOST('cancel')) {
			...
			if ($ret > 0) {
				...
				$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
				// Asunto y mensaje
				$subject="Mensaje de ticket actualizado";
				$message="El mensaje del ticket $object->ref ha sido actualizado,eche un vistazo al nuevo mensaje:
				\n$object->message";
		...
		if ($object->fetch(GETPOST('id', 'int'), GETPOST('track_id', 'alpha')) >= 0) {
			...
			if ($res) {
				$status_names = array(
					...
					3 => 'En Progreso',
					4 => 'En Espera',
					5 => 'Esperando Comentarios'
				);
				$old_status_name = $status_names[$old_status];
				...
				//no leido asignado....
				// Correo electronico a quien se envia

				// Correo electronico de quien envia
				$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
				// Asunto y mensaje
				$subject="Estado de ticket actualizado";
				$message="Le informamos que el ticket nº $object->ref ha cambiado de $old_status_name a $new_status_name \n";
				$message .= "Mensaje inicial:\n";
				$message .= $object->message;
	...
	if ($action == "change_property" && GETPOST('btn_update_ticket_prop', 'alpha') && $user->rights->ticket->write) {
		...
		if ($ret > 0) {
			...
			$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
			// Asunto y mensaje
			$subject="Gravedad/Tipo/Categoria de ticket actualizada";
			$message="Le informamos que la gravedad y/o categoria y/o el tipo del ticket $object->ref han sido actualizadas a:
			\nGravedad: ".$langs->trans($langs->getLabelFromKey($db, $object->severity_code, 'c_ticket_severity', 'code', 'label')).
			"\nCategoria:".$object->category_label = $langs->trans($langs->getLabelFromKey($db, $object->category_code, 'c_ticket_category', 'code', 'label')).
			"\nTipo:".$langs->trans($langs->getLabelFromKey($db, $object->type_code, 'c_ticket_type', 'code', 'label'));
	...
	// MIRAR ESTO PARA ENVIAR CORREO CADA VEZ QUE AÑADA MENSAJE
...
if ($action == 'create' || $action == 'presend') {
	...
	if ($res > 0) {
		...
		if ($action == 'presend' || $action == 'presend_addmessage') {
			...
			$from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', array(",")).' <'.$conf->global->MAIN_INFO_SOCIETE_MAIL.'>';
			// Asunto y mensaje
			$fk_user_create = $formticket->param['fk_user_create'];
			$sql = "SELECT firstname, lastname FROM khns_user WHERE rowid = $fk_user_create";
			$res = $db->query($sql);
			$row = $res->fetch_assoc();
			$nombre = $row["firstname"];
			$apellidos = $row["lastname"];
			$messageForm = GETPOST('message', 'restricthtml');

			$subject="Mensaje nuevo añadido a ticket";
			$message = "Un nuevo mensaje de $nombre $apellidos ha sido añadido al ticket nº $object->ref\n" ;
			$message .= "Mensaje inicial:\n";
			$message .= $object->message;
