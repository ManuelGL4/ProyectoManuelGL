...
if (!GETPOST('confirmmassaction', 'alpha') && $massaction != 'presend' && $massaction != 'confirm_presend' && $massaction != 'confirm_generateinvoice' && $massaction != 'confirm_generateinvoicej') { $massaction = ''; }
...
//esta funcion obtiene el numero de las diferentes jornadas en base a las seleccionadas
function getJornadas($seleccionados, $db, $userid){
	$jornadas = array();
	$sql = "SELECT task_date, fk_user FROM ".MAIN_DB_PREFIX."projet_task_time WHERE rowid IN (";
	...
	//Todos los tiempos del mismo dia y del mismo usuario hacen una jornada
...
if ($action == 'confirm_generateinvoice')
{
	...
	if (!($projectstatic->thirdparty->id > 0)) {
		...
		$sql = "SELECT fk_account, mode_reglement, cond_reglement FROM ".MAIN_DB_PREFIX."societe ";
		$sql.= "WHERE rowid = ".$tmpinvoice->socid."";
		$resultado = $db->query($sql);
		$datos = $db->fetch_object($resultado);

		$tmpinvoice->cond_reglement_id = $datos->cond_reglement;
		$tmpinvoice->mode_reglement_id = $datos->mode_reglement;
		$tmpinvoice->fk_account = $datos->fk_account;
...
}elseif ($action == 'confirm_generateinvoicej')
{
	if (!empty($projectstatic->socid)) $projectstatic->fetch_thirdparty();
		...
		if (!$error)
		{
			if ($generateinvoicemode == 'onelineperuser') {
				...
				foreach ($arrayoftasks as $userid => $value)
				{
					...
					// Calculo de jornadas seleccionadas
					$qtyhour = getJornadas($toselect, $db, $userid);
					$qtyhourtext = $qtyhour . " jornada(s) de " . $username;
				...
				foreach ($arrayoftasks as $timespent_id => $value)
				{
					...
					// Calculo de jornadas seleccionadas
					$qtyhour = getJornadas($toselect, $db);
					$qtyhourtext = $qtyhour . " jornada(s)";
...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0)
{
	...
	if ($projectstatic->usage_bill_time)
	{
		$arrayofmassactions = array(
			'generateinvoice'=>$langs->trans("Facturar por horas"),
			'generateinvoicej'=>$langs->trans("Facturar por jornadas")
		...
		if (in_array($massaction, array('presend', 'predelete', 'generateinvoicej'))) $arrayofmassactions = array();
	...
	if ($projectstatic->id > 0)
	{
		...
		elseif ($massaction == 'generateinvoicej' && $user->rights->facture->lire) print '<input type="hidden" name="action" value="confirm_generateinvoicej">';
		...
		if ($massaction == 'generateinvoice' || $massaction == 'generateinvoicej')
		{
	...
	//fecha, usuario, label, nota, horas
	//Si aparecen resultados en la tabla despues de listar con la lupa
	$users = array();
	$fechas = array();
	...
	{
		print "<div class='center'>";
		print "<form action='./excel.php' method='post'>";
		for($i = 0; $i < count($tasks); $i++){
			$nombre = "";
			if(strlen($tasks[$i]->firstname) > 0){
				$nombre = $tasks[$i]->firstname . " " . $tasks[$i]->lastname;
			}else{
				$nombre = $tasks[$i]->lastname;
			}
			array_push($users, $nombre);
		}
		...
		//Fecha
		print "<input type='hidden' name='dates' value='";
		for($i = 0; $i < count($tasks); $i++){
			print $tasks[$i]->task_date;
			if($i < count($tasks)-1){
				print ",";
			}
		}
		...
		//Label
		print "<input type='hidden' name='labels' value='";
		for($i = 0; $i < count($tasks); $i++){
			print $tasks[$i]->label;
			if($i < count($tasks)-1){
				print ",";
		...
		//Horas
		print "<input type='hidden' name='hours' value='";
		for($i = 0; $i < count($tasks); $i++){
			print segundos_tiempo($tasks[$i]->task_duration);
			if($i < count($tasks)-1){
				print ",";
			}
		}
		...
		print "<input type='submit' class='button reposition' name='excel' value='Generar .xls'>";
		print "</form>";
		print "</div>";
	}
}
