...
class Mo extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		$repa = $_POST['ocultoRepa'];
		$consu = $_POST['ocultoConsu'];
		$cantidadF = $_POST['ocultoQty'];
		$averia = $_POST['ocultoAveria'];
		$equipo = $_POST['ocultoEquipo'];

		print "Cantidad".$cantidadF;
		...
		if ($repa != "") {
			...
			$sqlUltimoId = " SELECT MAX(rowid) as maximo FROM ".MAIN_DB_PREFIX."mrp_production ";

			$resultUltimoId = $this->db->query($sqlUltimoId);
			$ultimoId = $this->db->fetch_object($resultUltimoId);

			/*//SACAMOS EL WAREHOUSE
			$sqlWare = " SELECT fk_warehouse FROM ".MAIN_DB_PREFIX."mrp_mo ";
			$sqlWare.= " WHERE fk_mo = $ultimoId->maximo ";

			print $sqlWare;

			$resultWare = $this->db->query($sqlWare);
			$ware = $this->db->fetch_object($resultWare);*/

			//PRUEBA DE UPDATE PARA QUE SEA TOPRODUCE
			$sqlUpdate = " UPDATE ".MAIN_DB_PREFIX."mrp_production ";
			$sqlUpdate.= " set role = 'toproduce' ";
			$sqlUpdate.= " WHERE rowid = $ultimoId->maximo ";

			$modificado = $this->db->query($sqlUpdate);

			//PRUEBA DE INSERCIÓN
			$sqlInsert = " INSERT INTO ".MAIN_DB_PREFIX."mrp_production ";
			$sqlInsert.= " (fk_mo, position, fk_product, qty, qty_frozen, disable_stock_change, role) ";
			$sqlInsert.= " VALUES ";
			$sqlInsert.= " ($idcreated, 2, $consu, $cantidadF, 0, 0, 'toconsume') ";

			print $sqlInsert;
			//die;

			$creado = $this->db->query($sqlInsert);

			//PRUEBA DE INSERCIÓN EN EXTRAFIELDS
			$sqlInsert2 = " INSERT INTO ".MAIN_DB_PREFIX."mrp_mo_extrafields_new ";
			$sqlInsert2.= " (fk_mo, fk_averia, closed) ";
			$sqlInsert2.= " VALUES ";
			$sqlInsert2.= " ($idcreated, $averia, 0) ";

			//print $sqlInsert2;
			//die;

			$creado2 = $this->db->query($sqlInsert2);
			//die;
			...
			$sqlUpdate.= " SET ot = 1 ";
			$sqlUpdate.= " WHERE rowid = ".$equipo;

			$creado3 = $this->db->query($sqlUpdate);

		} else {

			//PRUEBA DE INSERCIÓN EN EXTRAFIELDS
			$sqlInsert2 = " INSERT INTO ".MAIN_DB_PREFIX."mrp_mo_extrafields_new ";
			$sqlInsert2.= " (fk_mo, closed) ";
			$sqlInsert2.= " VALUES ";
			$sqlInsert2.= " ($idcreated, 0) ";
