...
// Both test must be present to be compatible with all browsers {
    ...
    $ls_date_time_event_year = GETPOST('ls_date_time_event_year', 'alpha');
    $ls_event_location_ref = GETPOST('ls_event_location_ref', 'alpha');
    $ls_event_type = GETPOST('ls_event_type', 'alpha');
...
switch($action) {
    ...
    case 'delete_records':
        ...
        if ($token!="" && $id!="") {
            ...
            $sql="DELETE FROM ".MAIN_DB_PREFIX."attendance_event WHERE token LIKE '%".$token."%' and event_type IN ( 1,3 ) and rowid !=".$id."";
            
            $respuesta = $db->query($sql);

            if ($respuesta) {

                setEventMessage('Los tiempos fueron ajustados', 'mesgs');

            }else{

                setEventMessage('Ningun registro fue eliminado', 'warnings');

            }

        }else{
            setEventMessage('Valores introducidos no válidos', 'warnings');
        }
    break;
}
        ...
        jQuery("#mybutton").click(function()
{
    ...
    //pass the search criteria 
        if($ls_date_time_event_day)$sqlwhere .= ' AND DAY(t.date_time_event) = "'.$ls_date_time_event_day."'";
        if($ls_date_time_event_month)$sqlwhere .= ' AND MONTH(t.date_time_event) = "'.$ls_date_time_event_month."'";
        if($ls_date_time_event_year)$sqlwhere .= ' AND DATE(t.date_time_event) = "'.$ls_date_time_event_year.'"';
...
if(empty($conf->global->MAIN_DISABLE_FULL_SCANLIST)) {
    ...
    if($resql) {
        ...
        if(!empty($ls_date_time_event_day))        $param .= '&ls_date_time_event_day='.urlencode($ls_date_time_event_day);
        ...
        print '<input class = "flat" type = "date"   name = "ls_date_time_event_year" value = "'.$ls_date_time_event_year.'">';
        print '</td>';
...
//                print "<td>".print_generic('third_party', 'rowid', $obj->fk_third_party, 'rowid', 'description')."</td>";
                ...
                $duration = ($obj->date_time_event_start <> "") ? ($db->jdate($obj->date_time_event) - $db->jdate($obj->date_time_event_start)) : 0;
                echo '<script>
                document.addEventListener("DOMContentLoaded", function() {
                    let durationInSeconds = ' . $duration . ';
                    let formattedDuration = formatTime(durationInSeconds);
                    console.log(formattedDuration); // Puedes usar esta variable en tu código JS
                });
                </script>';
                
                print "<td>".formatTime($duration, 0)."</td>";
                print '<td><a href = "AttendanceEventAdmin.php?action=delete&id='.$obj->rowid.'">'.img_delete().'</a></td>';
                print '<td><a href = "AttendanceEventAdmin.php?action=delete_records&id='.$obj->rowid.'&token='.$obj->token.'">'.img_edit().'</a></td>';
                print "</tr>";
            }
            $i++;
        }
