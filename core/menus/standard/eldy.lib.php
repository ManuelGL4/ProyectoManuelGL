...
function print_left_eldy_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null, $type_user = 0)
{
	...
	if ($mainmenu) {	// If this is empty, loading hard coded menu and loading personalised menu will fail
		...
		if ($mainmenu == 'home') {
			...
			//NUEVOS MENÚS
			/*$newmenu->add("../core/iva.php?mainmenu=home&amp;leftmenu=iva", $langs->trans("Info IVA"), 0, $user->admin, '', $mainmenu, 'iva', 0, '', '', '', '<i class="fa fa-book fa-fw paddingright pictofixedwidth"></i>');
				if ($usemenuhider || empty($leftmenu) || $leftmenu == "iva") {
				$newmenu->add("../core/iva.php?action=regimen&leftmenu=iva", $langs->trans("Régimen General"), 2, $user->rights->user->user->lire || $user->admin);
				$newmenu->add("../core/iva.php?action=tipo&leftmenu=iva", $langs->trans("Tipos de IVA"), 2, $user->rights->user->user->lire || $user->admin);
			}*/
		}
		...
		*/
		...
		if ($mainmenu == 'tools') {
			...
			/*if (!empty($conf->import->enabled)) {
				$langs->load("Prueba");
				$newmenu->add("/core/pruebaimportacion.php", $langs->trans("Importación prueba"), 0, $user->rights->import->run, '', $mainmenu, 'import', 0, '', '', '', img_picto('', 'technic', 'class="paddingright pictofixedwidth"'));
			}*/
		}
