...
class TimesheetReport
{
        ...
        if ($resql) {
            ...
            while ($i < $numTaskTime) {
                ...
                $resArray[$i] = array(
                    ...
                    'dateDisplay' => dol_print_date($this->db->jdate($obj->task_date), 'dayhour'),
    ...
    {
        //Consulta Intervalos de tiempo
        $fecha_start = date('Y-m-d', $this->startDate);
        $fecha_stop = date('Y-m-d', $this->stopDate);
        $user_id = implode("','", $this->userid);

        $sql = " SELECT firstname as nombre, lastname as apellido, GROUP_CONCAT(date_time_event) as fecha";
        $sql.= " ,min(ae.date_time_event) as datemin, max(ae.date_time_event) as datemax";
        $sql.= " FROM ".MAIN_DB_PREFIX."attendance_event as ae INNER JOIN ".MAIN_DB_PREFIX."user as u ON ae.fk_userid=u.rowid";
        $sql.= " where date(date_time_event) >= '$fecha_start' and date(date_time_event) <= '$fecha_stop'";
        $sql.= " and event_type IN ( 2,3 )";
        $sql.= " and u.rowid = $user_id";
        $sql.= " group by cast(ae.date_time_event as date)";
        
        $respuesta = $this->db->query($sql);

        $arrayReportDiario = array();
        ...
        if ($sql) {
            ...
            while ($i < $num) {
                $objinicio = $this->db->fetch_object($respuesta);

                $fechas = explode(",", $objinicio->fecha);
                sort($fechas);

                $duraciontotal = 0;

                for ($y=0; $y < count($fechas); $y=$y+2) { 
                    
                    $origin = new DateTimeImmutable($fechas[$y]);
                    $target = new DateTimeImmutable($fechas[$y+1]);
                    $diff = $target->getTimestamp() - $origin->getTimestamp();
                    $duraciontotal += $diff;
                    
                }
                
                $duraciontotal = formatTime($duraciontotal, 0);
                
                $dia = date("Y-m-d", strtotime($objinicio->datemin));
                $fecha_inicio = date("H:i", strtotime($objinicio->datemin));
                $fecha_fin = date("H:i", strtotime($objinicio->datemax));

                $arrayReportDiario[] = array(
                    'nombre' => $objinicio->nombre,
                    'apellido' => $objinicio->apellido,
                    'dia' => $dia,
    ...
    /**
     * Get the report of the time task, NO ATTENDANCE EVENT 
     */
    public function getReportProject()
    {
        $startDate = date('Y-m-d', $this->startDate);
        $stopDate = date('Y-m-d', $this->stopDate);
        $userId = implode("','", $this->userid);

        $sql = " SELECT u.firstname, u.lastname, SUM(ptt.task_duration) as task_duration, ptt.task_date,";
        $sql.= " MIN(TIME(ptt.task_datehour)) as date_start, MAX(TIME(ptt.task_datehour)) as date_end,";
        $sql.= " GROUP_CONCAT(ptt.note) as notes, GROUP_CONCAT(task_duration) as durations";
        $sql.= " FROM ".MAIN_DB_PREFIX."projet_task_time as ptt ";
        $sql.= " INNER JOIN ".MAIN_DB_PREFIX."user as u ON ptt.fk_user = u.rowid";
        $sql.= " WHERE ptt.task_date >= '$startDate' and ptt.task_date <= '$stopDate'";
        ...
        if ($response) {
            ...
            while ($objResponse = $this->db->fetch_object($response)) {

                $note = '';
                
                if ($objResponse->notes != '') {
                    
                    if (strpos($objResponse->notes,",") != false) {
                        
                        $notes = explode(",",$objResponse->notes);
                        
                        $durations = explode(",",$objResponse->durations);
        
                        $key = array_search(max($durations),$durations);
    
                        $note = $notes[$key];

                    }else {

                        $note = $objResponse->notes;
                    }
                ...
                $arrayReport[] = array(
                    'nombre' => $objResponse->firstname,
                    'apellido' => $objResponse->lastname,
                    'dia' => $objResponse->task_date,
                    'fecha_inicio' => $objResponse->date_start,
                    'fecha_fin' => $objResponse->date_end,
                    'duraciontotal' => gmdate("H:i:s",$objResponse->task_duration),
                    'note' => $note,
                    'sign' => ''
                );
    ...
    public function getReportTareas()
    {
        //Consulta Inicio Tarea
        $fecha_start = date('Y-m-d', $this->startDate);
        $fecha_stop = date('Y-m-d', $this->stopDate);
        $user_id = implode(",", $this->userid);

        $inicio_tareas = "SELECT firstname as nombre, lastname as apellido, ref as tarea, ae.note as nota, date_time_event as fecha_inicio 
        FROM ".MAIN_DB_PREFIX."attendance_event as ae JOIN ".MAIN_DB_PREFIX."user as u ON ae.fk_userid=u.rowid JOIN ".MAIN_DB_PREFIX."projet_task as pt on ae.fk_task=pt.rowid
        where event_type = 2 and date(date_time_event) >= '$fecha_start' 
        and date(date_time_event) <= '$fecha_stop' ";

        if (!empty($user_id)) {
            $inicio_tareas .= '  and u.rowid in ('.$user_id.') group by date_time_event ';
        }

        $respuestainicio = $this->db->query($inicio_tareas);

        //Consulta Fin Tarea
        $fin_tareas = "SELECT firstname as nombre, lastname as apellido, ref as tarea, ae.note as nota, date_time_event as fecha_fin 
        FROM ".MAIN_DB_PREFIX."attendance_event as ae JOIN ".MAIN_DB_PREFIX."user as u ON ae.fk_userid=u.rowid JOIN ".MAIN_DB_PREFIX."projet_task as pt on ae.fk_task=pt.rowid
        where event_type = 3 and date(date_time_event) >= '$fecha_start' 
        and date(date_time_event) <= '$fecha_stop' ";
        var_dump($fin_tareas);die;
        if (!empty($user_id)) {
            $fin_tareas .= '  and u.rowid in ('.$user_id.') group by date_time_event ';
        }

        $respuestafin = $this->db->query($fin_tareas);

        $arrayFechasTareas = array();

        if ($inicio_tareas && $respuestafin) {
            $num = $this->db->num_rows($respuestafin);
            $i = 0;

            while ($i < $num) {
                $objinicio = $this->db->fetch_object($respuestainicio);
                $objfin = $this->db->fetch_object($respuestafin);
                $fecha = substr($objinicio->fecha_inicio, 0, 11);
                $fecha_inicio = date("H:i", strtotime($objinicio->fecha_inicio));
                $fecha_fin = date("H:i", strtotime($objfin->fecha_fin));
                $arrayFechasTareas[$i] = array(
                    'nombre' => $objinicio->nombre,
                    'apellido' => $objinicio->apellido,
                    'tarea' => $objinicio->tarea,
                    'nota' => $objfin->nota,
                    'dia' => $fecha,
                    'fecha_inicio' => $fecha_inicio,
                    'fecha_fin' => $fecha_fin 
    ...
    */
    ...
    public function buildFile($model = 'excel2017', $save = false)
    {
        ...
        if (is_array($resArray)) {
            ...
            //DML
            //Aqui se recoge el nombre del Fichero y la extensión
            //$filename = "report.".$objmodel->getDriverExtension();
            $filename = "reporte_horas." . "xml";
            ...
            if ($result >= 0) {
                ...
                for ($i = count($resArray) - 1; $i > 0; $i--) {

                    if ($resArray[$i + 1]["dateDisplay"] !== $resArray[$i]["dateDisplay"]) {
                        $arrayImprimir[] = $resArray[$i];
                    }
                }
    ...
    public function buildFileDiario($model = 'excel2017', $save = false)
    {
        if ($model == 'excel2017' || $model == 'excel') {
            if (!(extension_loaded('zip') && extension_loaded('xml'))) {
                $this->error = "missing php extention(xml or zip)";
        ...
        $arrayTitle = array('nombre' => 'Nombre', 'apellido' => 'Apellido', 'dia' => 'Año-Mes-Día', 'fecha_inicio' => 'Hora Entrada', 'fecha_fin' => 'Hora Salida', 'duraciontotal' => 'Tiempo Trabajado', 'note' => 'Nota', 'sign' => 'Firma Trabajador/a',);
        $arrayTypes = array('nombre' => 'TextAuto', 'apellido' => 'TextAuto', 'dia' => 'TextAuto', 'fecha_inicio' => 'Date', 'fecha_fin' => 'Date', 'duraciontotal' => 'TextAuto', 'note' => 'TextAuto', 'sign' => 'TextAuto',);
        $arraySelected = array('nombre' => 'Nombre', 'apellido' => 'Apellido', 'dia' => 'Año-Mes-Día', 'fecha_inicio' => 'Hora Entrada', 'fecha_fin' => 'Hora Salida', 'duraciontotal' => 'Tiempo Trabajado', 'note' => 'Nota', 'sign' => 'Firma Trabajador/a',);

        //$resArray = $this->getReportDiario();
        $resArray = $this->getReportProject();
    ...
    public function buildFileComplete($model = 'excel2017', $save = false)
    {
        if ($model == 'excel2017' || $model == 'excel') {
            if (!(extension_loaded('zip') && extension_loaded('xml'))) {
                $this->error = "missing php extention(xml or zip)";
        ...
        $arrayTitle = array('nombre' => 'Nombre', 'apellido' => 'Apellido', 'dia' => 'Año-Mes-Día', 'tarea' => 'Tarea', 'fecha_inicio' => 'Hora Inicio', 'fecha_fin' => 'Hora Fin', 'nota' => 'Nota',);
        $arrayTypes = array('nombre' => 'TextAuto', 'apellido' => 'TextAuto', 'dia' => 'TextAuto', 'tarea' => 'TextAuto', 'fecha_inicio' => 'Date', 'fecha_fin' => 'Date', 'nota' => 'TextAuto',);
        $arraySelected = array('nombre' => 'Nombre', 'apellido' => 'Apellido', 'dia' => 'Año-Mes-Día', 'tarea' => 'Tarea', 'fecha_inicio' => 'Hora Inicio', 'fecha_fin' => 'Hora Fin', 'nota' => 'Nota',);

        $resArray = $this->getReportTareas();
        ...
        if (is_array($resArray)) {
            ...
            //Aqui se recoge el nombre del Fichero y la extensión
            //$filename = "report.".$objmodel->getDriverExtension();
            $filename = "reporte_horas_completo." . "xls";
