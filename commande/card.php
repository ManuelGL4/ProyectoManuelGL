		...
		if (!$error) {
			...
			if ($object_id > 0 && !$error) {
				...
				if (isset($_POST['proyecto']) && ($_POST['proyecto'] != "")) {
					...
					header('Location: '.$_SERVER["PHP_SELF"].'?id='.$object_id.'&idpro='.$idpro.'');
				} else {
...
if ($action == 'create' && $usercancreate) {
	...
	print '<input type="hidden" name="proyecto" value="'.$idpro.'">';
			...
			if (isset($_GET['idpro'])) {
				...
				$(document).ready(function() {
					$("#socid").change(function() {
						var socid = $(this).val();
						// Modifica la acción del formulario para incluir la variable action
						$("form[name=crea_commande]").attr("action", "'.$_SERVER["PHP_SELF"].'?action=create&idpro='.$idpro.'");
						// Envía el formulario
		...
		*/
		...
		if ($idpro != "") {
			if (empty($object->lines)) {
				...
				//VER SI TIENE IVA DEFINIDO
				$sqlIva = " SELECT se.porc_iva, s.remise_client FROM ".MAIN_DB_PREFIX."societe_extrafields se ";
				$sqlIva.= " INNER JOIN ".MAIN_DB_PREFIX."societe s ON s.rowid = se.fk_object ";
				$sqlIva.= " WHERE se.fk_object = ".$object->socid." ";
				$resultIva = $db->query($sqlIva);
				$iva = $db->fetch_object($resultIva);
				...
				if ($iva->remise_client != "") {
					$descuento = $iva->remise_client;
				} else {
					$descuento = 0;
				}

				//PARA VER EL DESCUENTO DE LA OFERTA
				$sqlOferta = " SELECT discount_offer FROM ".MAIN_DB_PREFIX."projet_extrafields ";
				$sqlOferta.= " WHERE fk_object = ".$idpro." ";
				$resultOferta = $db->query($sqlOferta);
				$oferta = $db->fetch_object($resultOferta);

				if ($oferta->discount_offer != "") {
					$descuento+= $oferta->discount_offer;
				}

				//SACAMOS TODOS LOS DATOS DE PRODUCTOS DEL PROYECTO
				$sqlProductos = " SELECT * FROM ".MAIN_DB_PREFIX."proyectos_oferta_materiales ";
				$sqlProductos.= " WHERE fk_project = ".$idpro." ";
				...
				while ($pro = $db->fetch_object($resultProductos)) {
					...
					if (($pro->discount > 0)) {
						...
						$descontar = ($pro->price * $pro->discount) / 100;
						$pro->price = $pro->price - $descontar;

					}

					$result = $object->addline("".$pro->description."", $pro->price, $pro->quantity, $ivaf, "", "", $pro->fk_product, $descuento, "", 0, "HT", "", "", "", "", "", "", "", "", "", "", "", "", '', "", "");
				}
