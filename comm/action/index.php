...
if (!empty($conf->use_javascript_ajax)) {	// If javascript on
	...
	// Birthdays
	$s .= '<div class="nowrap inline-block"><input type="checkbox" id="check_birthday" name="check_birthday" class="check_birthday"><label for="check_birthday"> <span class="check_birthday_text">'.$langs->trans("Cumpleaños de usuarios").'</span></label> &nbsp; </div>';
...
if ($showbirthday) {
	// Add events in array
	// CAMBIOS HECHOS
	//$sql = 'SELECT u.rowid, u.lastname, u.firstname, u.birth';
	//$sql .= ' FROM '.MAIN_DB_PREFIX.'user as u';
	//$sql .= ' WHERE fk_user_creat='.$user->id.'';
	//$sql .= " AND u.entity IN (".getEntity('user').")";
	$sql = "SELECT u.rowid, u.lastname, u.firstname, u.birth ";
	$sql.= "FROM ".MAIN_DB_PREFIX."user u ";
	$sql.= "WHERE fk_user_creat IS NULL ";
	$sql.= "AND u.entity IN (0,1) ";
	if ($action == 'show_day') {
		$sql .= ' AND MONTH(birth) = '.((int) $month);
		$sql .= ' AND DAY(birth) = '.((int) $day);
	} else {
		$sql .= ' AND MONTH(birth) = '.((int) $month);
	}
	$sql .= ' ORDER BY birth';
	...
	if ($resql) {
		...
		while ($i < $num) {
			...
			$datebirth = dol_stringtotime($obj->birth, 1);
			...
			foreach ($eventarray[$daykey] as $index => $event) {
				...
				if ($i < $maxprint || $maxprint == 0 || !empty($conf->global->MAIN_JS_SWITCH_AGENDA)) {
					...
					if ($event->type_code == 'BIRTHDAY') { 			// It's birthday calendar
						//print "ESTO: ".$event->getNomUrl(1, $maxnbofchar, 'cal_event', 'birthday', 'user');
						$consulta = "SELECT lastname, firstname FROM ".MAIN_DB_PREFIX."user WHERE rowid = ".$event->id;

						$resultConsulta = $db->query($consulta);
						$user = $db->fetch_object($resultConsulta);

						print "<a href=../../user/card.php?id=".$event->id."><b>Cumpleaños</b>: ".$user->firstname." ".$user->lastname."</a>";
						//print "<a href='www.inventado.com'>LINK</a>";
					} elseif ($event->type_code == 'HOLIDAY') {		// It's holiday calendar
						$tmpholiday->fetch($event->id);

						$tmpholiday->getNomUrl(1);
