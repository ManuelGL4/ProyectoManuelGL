...
if ($action == 'create') {
	...
	print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	print '<input type="text" style="width:600px" readonly id="score_provider" value="">';
	print '<script>
			$(document).ready(function() {
				let input=document.getElementById("score_provider");
				$("#socid").change(function() {
					var socid = $(this).val();
					
					fetch("scoreProvider.php?id="+socid)
					.then((response) => response.json() )
					.then((data) => input.value="Nota calidad de suministros: "+data["score_supplies"]+" | Nota cumplimiento de plazos: "+data["score_times"] );
				});
			});
			</script>';
	print '</td>';
