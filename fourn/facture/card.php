...
if (empty($reshook)) {
	...
	if ($action == 'settransportmode' && ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer)) {
		...
		if (GETPOST('type') == FactureFournisseur::TYPE_STANDARD || GETPOST('type') == FactureFournisseur::TYPE_DEPOSIT) {
			...
			if (!$error) {
				...
				if (!$error && GETPOST('origin', 'alpha') && GETPOST('originid')) {
					...
					$id = $object->create($user);//Create supplier invoice HERE
					if ($id < 0) {
						$error++;
					}
				}
		...
		if ($error) {
			...
			$db->commit();
			//generate invoice document HERE
			if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) {
				$outputlangs = $langs;
				$result = $object->generateDocument($object->model_pdf, $outputlangs, $hidedetails, $hidedesc, $hideref);
				if ($result < 0) {
...
if ($action == 'create') {
	...
	*/
	...
	print $form->textwithpicto($langs->trans('Factura'), $htmltext);
	print '</td>';
	print '<td valign="top" colspan="2">';
	print '<input type="file" name="facture_base" accept=".xml">';
	print '</td>';
	print '</tr>';
