...
class Interventions extends DolibarrApi
{
	...
	*/
	 ...
	 * @return  array
	 */
	public function addDataWebForm($request = null)
	{
		try {

			$errors = $this->_validateWebForm($request);
			
			if(!empty($errors)) throw new Exception("Error en el formulario", 400);
			
			$query = ' INSERT INTO '.MAIN_DB_PREFIX.'faults';
			$query.= ' ( client, company_name, cif, description, name, phone, email )';
			$query.= ' VALUES ( "'.$request['fault']['client'].'", "'.$request['fault']['company_name'].'", "'.$request['fault']['cif'].'",';
			$query.= ' "'.$request['fault']['description'].'", "'.$request['contact']['name'].'", "'.$request['contact']['phone'].'",';
			$query.= ' "'.$request['contact']['email'].'" )';
			
			$result = $this->db->query($query);

			if (!$result) throw new Exception("Error al añadir los datos", 500);

		} catch (Exception $e) {
			return ['status' => $e->getCode(), 'message' => $e->getMessage(), 'errors' => $errors];
		}
		 
		return ['status' => 200,'message' => 'Exito'];
	}

	/**
	 * Validate fields before object
	 *
	 * @param array $data   Data to validate
	 * @return array
	 *
	 */
	private function _validateWebForm($request)
	{
		$errors = [];

		if ($request['fault']['client'] == null){
			$errors['fault']['client'] = "El cliente asociado a la avería es necesario";
		}
		if ($request['fault']['company_name'] == null){
			$errors['fault']['company_name'] = "La razón social asociada a la avería es necesaria";
		}
		if ($request['fault']['cif'] == null){
			$errors['fault']['cif'] = "El CIF asociado a la avería es necesario";
		}
		if ($request['fault']['description'] == null){
			$errors['fault']['description'] = "La descripción de la avería es necesaria";
		}

		if ($request['contact']['name'] == null){
			$errors['contact']['name'] = "El nombre del contacto es necesario";
		}
		if ($request['contact']['phone'] == null){
			$errors['contact']['phone'] =  "El teléfono del contacto es necesario";
		}
		if ($request['contact']['email'] == null){
			$errors['contact']['email'] =  "El email del contacto es necesario";
		}
